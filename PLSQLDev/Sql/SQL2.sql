...
SELECT T1.OPEN_SCHYY /* 개설학년도(PK1) */,
       ...
       NVL((SELECT SUM(LT_ASSIST_ASGN_RCNT)
             FROM COUR622
            WHERE OPEN_SCHYY = T1.OPEN_SCHYY
              AND OPEN_SHTM_FG = T1.OPEN_SHTM_FG
              AND OPEN_DETA_SHTM_FG = T1.OPEN_DETA_SHTM_FG
              AND ASGN_FG = 'U049500001'
              AND ASGN_BREU_FG = '100'
              AND DEPT_CD = T1.MNGT_DEPT_CD
              AND LT_ASSIST_TYPE_FG = T1.LT_ASSIST_TYPE_FG),
           0) AS RCNT /* 배정인원 */,
       SF_BSNS011_CODENM(T1.LT_ASSIST_TYPE_FG) AS LT_ASSIST_TYPE_KOR_NM /* 강의조교유형구분 */,
