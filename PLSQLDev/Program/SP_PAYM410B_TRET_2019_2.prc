CREATE OR REPLACE PROCEDURE SP_PAYM410B_TRET_2019_2
(
...
-- eterain 기부금처리시 사용 변수 20200205
V_TMP1               NUMBER(15)     := 0;                -- 기부금처리시 TEMP VAR
V_TMP2               NUMBER(15)     := 0;                -- 기부금처리시 TEMP VAR
V_STR1               VARCHAR2(1)    := 'F';              -- 기부금처리시 TEMP VAR
V_EXPAND_AMT         NUMBER(15)     := 0;                -- 기부금처리시 : 기준소득금액
V_GONGJE_TAX_AMT     NUMBER(15)     := 0;                -- 기부금처리시 : 세액공제대상합계
        ...
        DELETE FROM PAYM410 A
         WHERE A.YY             = IN_YY
           ...
           AND A.RPST_PERS_NO IN ( SELECT RPST_PERS_NO --2013년 추가, 기관별 처리.
                                     FROM PAYM420 B
                                    WHERE B.YY         = IN_YY
                                      AND B.YRETXA_SEQ = IN_YRETXA_SEQ  
                                      AND B.SETT_FG    = V_SETT_FG
                                      AND B.LABOR_SCHLS_FG = 'N' --근로장학생이 아닌 사람만 연말정산 계산
                                      AND NVL(B.SALY_DEPT_CD,' ') = NVL(IN_DEPT_CD, NVL(B.SALY_DEPT_CD,' '))  --2013년 추가, 기관별 처리.
                                     UNION
...
DBMS_OUTPUT.PUT_LINE('----- eterain start-t' );
            V_STR1 := 'F';
            -- eterain-01 20200205-기부금처리-정치자금기부금 
            V_CNTRIB_DUC_SUM_AMT20 := 0;
            V_TMP1 := 0;
            V_TMP2 := 0;
            V_CNTRIB_AMT_CYOV_AMT := 0;
            SP_PAYM410B_2019_DONATE_P( V_STR1, IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, IN_SETT_FG, REC.RPST_PERS_NO,
                                           V_LABOR_EARN_AMT,                                                         -- 근로소득금액
                                           V_TMP1,                                                                   -- 기준소득금액(내부계산처리)
                                           V_TMP2,                                                                   -- 한도(내부계산처리)      
                                           V_TDUC_DUC_TT_AMT,                                                        -- 산출세액(합산)   
                                           V_CAL_TDUC_TEMP_AMT,                                                      -- 산출세액(차감)   
                                           V_SPCL_DUC_AMT,                                                           -- 특별소득공제   
                                           V_STAD_TAXDUC_OBJ_AMT,                                                    -- 표준세액공제   
                                           V_CNTRIB_DUC_SUM_AMT20,                                                   -- 정치자금 : 공제대상금액-합계
...
DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_AMT -> ' || V_LABOR_EARN_AMT );
DBMS_OUTPUT.PUT_LINE('V_TMP1 -> ' || V_TMP1 );
DBMS_OUTPUT.PUT_LINE('V_TMP2 -> ' || V_TMP2 );
DBMS_OUTPUT.PUT_LINE('V_TDUC_DUC_TT_AMT -> ' || V_TDUC_DUC_TT_AMT );
DBMS_OUTPUT.PUT_LINE('V_CAL_TDUC_TEMP_AMT -> ' || V_CAL_TDUC_TEMP_AMT );
DBMS_OUTPUT.PUT_LINE('V_SPCL_DUC_AMT -> ' || V_SPCL_DUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_STAD_TAXDUC_OBJ_AMT -> ' || V_STAD_TAXDUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT20 -> ' || V_CNTRIB_DUC_SUM_AMT20 );
DBMS_OUTPUT.PUT_LINE('V_POLITICS_BLW_DUC_OBJ_AMT -> ' || V_POLITICS_BLW_DUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_POLITICS_BLW_TAXDUC_AMT -> ' || V_POLITICS_BLW_TAXDUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_POLITICS_EXCE_DUC_OBJ_AMT -> ' || V_POLITICS_EXCE_DUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_POLITICS_EXCE_TAXDUC_AMT -> ' || V_POLITICS_EXCE_TAXDUC_AMT );

            -- eterain-02 20200205-기부금처리-법정기부금 
            V_CNTRIB_DUC_SUM_AMT10 := 0;
            V_EXPAND_AMT := V_LABOR_EARN_AMT - V_CNTRIB_DUC_SUM_AMT20; -- 기준소득금액(근로소득금액 - 정치자금기부금(공제대상금액-합계))
            V_GONGJE_TAX_AMT := 0;
            V_TMP1 := 0;
            SP_PAYM410B_2019_DONATE_B( V_STR1, IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, IN_SETT_FG, REC.RPST_PERS_NO, V_BIZR_REG_NO,
                                           V_LABOR_EARN_AMT,                                                         -- 근로소득금액   
                                           V_EXPAND_AMT,                                                             -- 기준소득금액
                                           V_TMP1,                                                                   -- 한도(내부계산처리)               
                                           V_TDUC_DUC_TT_AMT,                                                        -- 산출세액(합산)   
                                           ...
                                           V_SPCL_DUC_AMT,                                                           -- 특별소득공제   
                                           V_STAD_TAXDUC_OBJ_AMT,                                                    -- 표준세액공제   
                                           V_CNTRIB_DUC_SUM_AMT10,                                                   -- 법정기부금 : 공제대상금액-합계
                                           V_GONGJE_TAX_AMT,                                                         -- 법정기부금 : 세액공제대상금액-합계   
                                           V_LABOR_TEMP_AMT,                                                         -- 공제대상금액-소득공제 대상 누적합계   
                                           V_CNTRIB_AMT_CYOV_AMT,                                                    --특별소득공제(기부금)
                                           IN_INPT_ID, IN_INPT_IP, OUT_RTN, OUT_MSG);                                           
            IF OUT_RTN <> '1' THEN
               RETURN;
            END IF;
            V_FLAW_CNTRIB_DUC_OBJ_AMT := V_CNTRIB_DUC_SUM_AMT10;                                                     --법정기부공제대상금액   
            V_FLAW_CNTRIB_TAXDUC_AMT := V_GONGJE_TAX_AMT;                                                            --법정기부세액공제액
            
DBMS_OUTPUT.PUT_LINE('----------------------------------------------------------------------------' );
DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_AMT -> ' || V_LABOR_EARN_AMT );
DBMS_OUTPUT.PUT_LINE('V_EXPAND_AMT -> ' || V_EXPAND_AMT );
DBMS_OUTPUT.PUT_LINE('V_TMP1 -> ' || V_TMP1 );
DBMS_OUTPUT.PUT_LINE('V_TDUC_DUC_TT_AMT -> ' || V_TDUC_DUC_TT_AMT );
DBMS_OUTPUT.PUT_LINE('V_CAL_TDUC_TEMP_AMT -> ' || V_CAL_TDUC_TEMP_AMT );
DBMS_OUTPUT.PUT_LINE('V_SPCL_DUC_AMT -> ' || V_SPCL_DUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_STAD_TAXDUC_OBJ_AMT -> ' || V_STAD_TAXDUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT10 -> ' || V_CNTRIB_DUC_SUM_AMT10 );
DBMS_OUTPUT.PUT_LINE('V_GONGJE_TAX_AMT -> ' || V_GONGJE_TAX_AMT );
DBMS_OUTPUT.PUT_LINE('V_LABOR_TEMP_AMT -> ' || V_LABOR_TEMP_AMT );
            
            -- eterain-03 20200205-기부금처리-우리사주기부금 
            V_CNTRIB_DUC_SUM_AMT42 := 0;
            V_EXPAND_AMT := V_EXPAND_AMT - V_CNTRIB_DUC_SUM_AMT10; -- 기준소득금액(법정:기준소득금액 - 법정(공제대상금액-합계))   
            V_GONGJE_TAX_AMT := 0;
            V_TMP1 := 0;
            SP_PAYM410B_2019_DONATE_W( V_STR1, IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, IN_SETT_FG, REC.RPST_PERS_NO,
                                           V_LABOR_EARN_AMT,                                                         -- 근로소득금액
                                           V_EXPAND_AMT,                                                             -- 기준소득금액         
                                           V_TMP1,                                                                   -- 한도(내부계산처리)                  
                                           V_TDUC_DUC_TT_AMT,                                                        -- 산출세액(합산)   
                                           ...
                                           V_SPCL_DUC_AMT,                                                           -- 특별소득공제   
                                           V_STAD_TAXDUC_OBJ_AMT,                                                    -- 표준세액공제   
                                           V_CNTRIB_DUC_SUM_AMT42,                                                   -- 우리사주기부금 : 공제대상금액-합계    
                                           V_GONGJE_TAX_AMT,                                                         -- 법정기부금 : 세액공제대상금액-합계
                                           IN_INPT_ID, IN_INPT_IP, OUT_RTN, OUT_MSG);
            IF OUT_RTN <> '1' THEN
               RETURN;
...
DBMS_OUTPUT.PUT_LINE('V_SPCL_DUC_AMT -> ' || V_SPCL_DUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_STAD_TAXDUC_OBJ_AMT -> ' || V_STAD_TAXDUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT42 -> ' || V_CNTRIB_DUC_SUM_AMT42 );
DBMS_OUTPUT.PUT_LINE('V_GONGJE_TAX_AMT -> ' || V_GONGJE_TAX_AMT );

            -- eterain-04 20200205-기부금처리-지정기부금 
            V_EXPAND_AMT := V_EXPAND_AMT - V_CNTRIB_DUC_SUM_AMT42; -- 기준소득금액(지정:기준소득금액 - 우리(공제대상금액-합계)) 
            V_GONGJE_TAX_AMT := 0;
            V_TMP1 := 0;
            SP_PAYM410B_2019_DONATE_G( V_STR1, IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, IN_SETT_FG, REC.RPST_PERS_NO, V_BIZR_REG_NO,
                                           V_LABOR_EARN_AMT,                                                         -- 근로소득금액
                                           V_EXPAND_AMT,                                                             -- 기준소득금액            
                                           V_TMP1,                                                                   -- 한도(내부계산처리)                  
                                           V_TDUC_DUC_TT_AMT,                                                        -- 산출세액(합산)    
                                           ...
                                           V_SPCL_DUC_AMT,                                                           -- 특별소득공제 
                                           V_STAD_TAXDUC_OBJ_AMT,                                                    -- 표준세액공제 
                                           V_CNTRIB_DUC_SUM_AMT4041,                                                 -- 지정기부금 : 공제대상금액-합계
                                           V_GONGJE_TAX_AMT,                                                         -- 법정기부금 : 세액공제대상금액-합계
                                           V_CNTRIB_DUC_SUM_AMT10,                                                   -- 법정기부금 : 공제대상금액-합계   
                                           V_CNTRIB_DUC_SUM_AMT42,                                                   -- 우리사주기부금 : 공제대상금액-합계
                                           V_APNT_CNTRIB40_DUC_OBJ_AMT,                                              -- 지정기부(종교외)공제대상금액
                                           V_APNT_CNTRIB41_DUC_OBJ_AMT,                                              -- 지정기부(종교)공제대상금액
                                           V_APNT_CNTRIB40_TAXDUC_AMT,                                               -- 지정기부(종교외)세액공제액
                                           V_APNT_CNTRIB41_TAXDUC_AMT,                                               -- 지정기부(종교)세액공제액                                           
                                           V_LABOR_TEMP_AMT,                                                         -- 공제대상금액-소득공제 대상 누적합계   
                                           V_CNTRIB_AMT_CYOV_AMT,                                                    --특별소득공제(기부금)
                                           IN_INPT_ID, IN_INPT_IP, OUT_RTN, OUT_MSG);
            IF OUT_RTN <> '1' THEN
               RETURN;
            END IF;
            V_APNT_CNTRIB_DUC_OBJ_AMT := V_CNTRIB_DUC_SUM_AMT4041;                                                   --지정기부공제대상금액   
            V_APNT_CNTRIB_TAXDUC_AMT := V_GONGJE_TAX_AMT;                                                            --지정기부세액공제액
...
DBMS_OUTPUT.PUT_LINE('V_SPCL_DUC_AMT -> ' || V_SPCL_DUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_STAD_TAXDUC_OBJ_AMT -> ' || V_STAD_TAXDUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT4041 -> ' || V_CNTRIB_DUC_SUM_AMT4041 );
DBMS_OUTPUT.PUT_LINE('V_GONGJE_TAX_AMT -> ' || V_GONGJE_TAX_AMT );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT10 -> ' || V_CNTRIB_DUC_SUM_AMT10 );
DBMS_OUTPUT.PUT_LINE('V_CNTRIB_DUC_SUM_AMT42 -> ' || V_CNTRIB_DUC_SUM_AMT42 );
DBMS_OUTPUT.PUT_LINE('V_APNT_CNTRIB40_DUC_OBJ_AMT -> ' || V_APNT_CNTRIB40_DUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_APNT_CNTRIB41_DUC_OBJ_AMT -> ' || V_APNT_CNTRIB41_DUC_OBJ_AMT );
DBMS_OUTPUT.PUT_LINE('V_APNT_CNTRIB40_TAXDUC_AMT -> ' || V_APNT_CNTRIB40_TAXDUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_APNT_CNTRIB41_TAXDUC_AMT -> ' || V_APNT_CNTRIB41_TAXDUC_AMT );
DBMS_OUTPUT.PUT_LINE('V_LABOR_TEMP_AMT -> ' || V_LABOR_TEMP_AMT );

DBMS_OUTPUT.PUT_LINE('----- eterain end-t' );
