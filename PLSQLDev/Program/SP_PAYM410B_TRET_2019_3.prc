CREATE OR REPLACE PROCEDURE SP_PAYM410B_TRET_2019_3
(
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 - 법정 Start');
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 - 지정 Start');
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 - 지정 - 종교단체기부금이 있는경우 Start');
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 - 지정 - 종교단체기부금이 없는경우 Start');
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 - 상단 End');
...
DBMS_OUTPUT.PUT_LINE('eterain 기부금 세액공제 전 결정세액 =====>>>>> ' || V_CAL_TDUC_TEMP_AMT || ' --------------------- 정상 결정세액산출');
                          ...
                          IF V_TMP10_AMT_2014 > 0 THEN
                              ...
                              SELECT V_TMP10_AMT_2014 - CNTRIB_GIAMT - CNTRIB_PREAMT
                               ...
                               WHERE YY             = IN_YY
                                 ...
                                 */
                          ELSE
                             /* ②금액이 0이하일 경우 ①금액(V_TMP10_AMT_2014) 공제 : 2014년이후 이월 법정기부금(년도순) */
                              V_TMP_STEP := 'U40_2014_2';
                              UPDATE PAYM436
                                 SET CNTRIB_GONGAMT = V_FLAW_CNTRIB_DUC_OBJ_AMT
                ...
                /* 1.지정기부금(40:종교외) 세액공제액 = 전체지정기부금세액공제액 * ( 지정(40:종교외) 대상금액 / 전체지정기부금공제대상액) [비율로 계산] */
                   ...
                   IF (V_FLAW_CNTRIB_DUC_OBJ_AMT + V_APNT_CNTRIB40_DUC_OBJ_AMT) > 10000000 THEN
                      IF V_FLAW_CNTRIB_DUC_OBJ_AMT = 0 THEN
                         ...
                         /* @VER.2019_3  고액기부금 기준 금액 변화(2천만원에서 1천만원으로 변경) */
                         V_APNT_CNTRIB40_TAXDUC_AMT := 10000000*0.15 + (V_APNT_CNTRIB40_DUC_OBJ_AMT - 10000000) * 0.3 ;
                      ELSE
                         ...
                         IF (V_FLAW_CNTRIB_DUC_OBJ_AMT > 10000000) THEN
                            ...
                            V_APNT_CNTRIB40_TAXDUC_AMT := V_APNT_CNTRIB40_DUC_OBJ_AMT * 0.3;
                         ELSE
...
DBMS_OUTPUT.PUT_LINE('eterain 시뮬레이션 아닐경우 Start');

DBMS_OUTPUT.PUT_LINE('eterain 법정기부금공제대상금액 ~ 2013 V_FLAW_CNTRIB_DUC_OBJ_AMT : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);   
...
DBMS_OUTPUT.PUT_LINE('eterain 법정기부금공제대상금액 2014 ~ IN_YY - 1 V_FLAW_CNTRIB_DUC_OBJ_AMT : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);   
                ...
                --eterain IF V_TMP10_AMT_2014 <= 0 THEN
                IF V_TMP10_AMT_2014 = 0 THEN
                ...
                /* 기부금 공제 순서 변경. 이월액 먼저 소멸되도록 변경됨 (@VER.2019_7)
                       (기존) 2013 이월 종교 외 지정 -> 2013 이월 종교 지정 -> 당해 종교 외 지정 -> 2014 이후 이월 종교 외 지정 -> 당해 종교 지정 -> 2014 이후 이월 종교 지정
                       (변경) 2013 이월 종교 외 지정 -> 2013 이월 종교 지정 -> 2014 이후 이월 종교 외 지정 -> 당해 종교 외 지정 -> 2014 이후 이월 종교 지정 -> 당해 종교 지정 */
DBMS_OUTPUT.PUT_LINE('eterain 법정기부금공제대상금액 2014 ~ IN_YY - 1 end');   

DBMS_OUTPUT.PUT_LINE('eterain 지정기부금공제대상금액  V_APNT_CNTRIB_DUC_OBJ_AMT,V_APNT_CNTRIB_TAXDUC_AMT : ' || V_APNT_CNTRIB_DUC_OBJ_AMT || ' , ' || V_APNT_CNTRIB_TAXDUC_AMT);                      
DBMS_OUTPUT.PUT_LINE('eterain 지정기부금공제대상금액  V_APNT_CNTRIB40_DUC_OBJ_AMT,V_APNT_CNTRIB40_TAXDUC_AMT : ' || V_APNT_CNTRIB40_DUC_OBJ_AMT || ' , ' || V_APNT_CNTRIB40_TAXDUC_AMT);                      
                ...
                ELSE
                  ...
                  BEGIN
                      ...
                      -- eterain V_TMP40_AMT := V_APNT_CNTRIB_DUC_OBJ_AMT;  --기부금 공제 순서 변경에 따른 초기값 셋팅 @VER.2019_7
                      -- 제거 V_TMP40_AMT := V_APNT_CNTRIB_DUC_OBJ_AMT;  --기부금 공제 순서 변경에 따른 초기값 셋팅 @VER.2019_7
                        ...
                        END;
                          ...
                          IF V_TMP40_AMT_2014 > 0 THEN
                              ...
                              UPDATE PAYM432
                                 ...
                                 --eterain SET CNTRIB_GONGAMT = CNTRIB_GIAMT - CNTRIB_PREAMT
                                 SET CNTRIB_GONGAMT = CASE WHEN V_TMP40_AMT_2014 > (CNTRIB_GIAMT - CNTRIB_PREAMT) THEN V_TMP40_AMT_2014 - (CNTRIB_GIAMT - CNTRIB_PREAMT)
                                                           ELSE CNTRIB_GIAMT - CNTRIB_PREAMT END
                                   --eterain , CNTRIB_OVERAMT = 0
                                   , CNTRIB_OVERAMT = CASE WHEN V_TMP40_AMT_2014 > (CNTRIB_GIAMT - CNTRIB_PREAMT) THEN V_TMP40_AMT_2014 - (CNTRIB_GIAMT - CNTRIB_PREAMT)
                                                           ELSE 0 END
                                   , CNTRIB_DESTAMT = 0
                               WHERE YY             = IN_YY
                                 AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                          ...
                          ELSE
                             /* ②금액이 0미만일 경우 (V_TMP40_AMT) 공제 : 2014년이후 이월 지정기부금(종교외) */
                              ...
                              UPDATE PAYM432
                                 --eterain SET CNTRIB_GONGAMT = V_TMP40_AMT
                                 SET CNTRIB_GONGAMT = V_TMP40_AMT_2014
                                   , CNTRIB_OVERAMT = CASE WHEN REC_40_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                                           THEN 0
                                                               
                                                           --eterain ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT
                                                           ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT_2014
                                                               
                                                      END
                                   , CNTRIB_DESTAMT = CASE WHEN REC_40_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                   
                                                           --eterain THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT
                                                           THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT_2014
...
----------------------------------
                   ...
                   BEGIN
                      /* ①:지정기부공제대상금액 - 당해 지정기부금(종교외) */
                      --eterain SELECT GREATEST(V_TMP40_AMT_2014 - CNTRIB_GIAMT, 0)
                      SELECT V_TMP40_AMT_2014 - CNTRIB_GIAMT
                        INTO V_TMP40_AMT
                        FROM PAYM432
                       WHERE YY             = IN_YY
                         AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                  ...
                  /* 당해 지정, 종교기부금 계산시 잔여기부금이 0보다 작은 경우 UPDATE 하지 않는다. 기존 로직 삭제 (@VER.2019_13) */
                  --eterain 아래 else 부분 사용으로 수정
                  ELSE
                  /* ①금액이 0미만일 경우 지정기부공제대상금액 공제 */
                      V_TMP_STEP := 'UU40_2';
                      UPDATE PAYM432
                         SET CNTRIB_GONGAMT = V_TMP40_AMT_2014
                           , CNTRIB_OVERAMT = CNTRIB_GIAMT - V_TMP40_AMT_2014
                       WHERE YY             = IN_YY
                         AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                         AND CNTRIB_YY      = IN_YY /*당해 기부금*/
                         AND SETT_FG        = V_SETT_FG
...
DBMS_OUTPUT.PUT_LINE('eterain 지정기부금공제대상금액  V_APNT_CNTRIB_DUC_OBJ_AMT,V_APNT_CNTRIB_TAXDUC_AMT : ' || V_APNT_CNTRIB_DUC_OBJ_AMT || ' , ' || V_APNT_CNTRIB_TAXDUC_AMT);                      
DBMS_OUTPUT.PUT_LINE('eterain 지정기부금공제대상금액  V_APNT_CNTRIB41_DUC_OBJ_AMT,V_APNT_CNTRIB41_TAXDUC_AMT : ' || V_APNT_CNTRIB41_DUC_OBJ_AMT || ' , ' || V_APNT_CNTRIB41_TAXDUC_AMT);                      
DBMS_OUTPUT.PUT_LINE('eterain 지정기부금공제대상금액  V_TMP40_AMT : ' || V_TMP40_AMT);                      
...
DBMS_OUTPUT.PUT_LINE('eterain ----- >> 기부금2019 하단 - End');
