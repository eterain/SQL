CREATE OR REPLACE PROCEDURE SNU.SP_PAYM410B_TRET_2019_OHH
(
...
---- richardoh 정치 소득공제 대상금액한도 : 근로소득금액 * 100%   한도 내에서 전액 소득공제....
                IF V_FLAW_CNTRIB_AMT > V_LMT_CNTRIB_AMT THEN
                ...
                V_LMT_CNTRIB_10_AMT  := GREATEST(0, V_LABOR_EARN_AMT - V_FLAW_CNTRIB_AMT); 
                --기부금공제대상 한도 금액
                                             ...
                                             IF V_TMP_AMT >= 0 THEN
                                                  UPDATE PAYM436
                                                     SET CNTRIB_OVERAMT = 0 --당년이월금액 
                                                       , CNTRIB_PREAMT  = REC_IWOL.CNTRIB_PREAMT + REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 전년까지의기부금액에서  이월액을 더한다)
                                                         ...
                                                         -- 당년 소멸금액 법이 어캐 바뀔지 모르니 2022년에..바꿔도 될듯   
                                                          UPDATE PAYM436
                                                             SET CNTRIB_OVERAMT = CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT)  --당년이월금액  CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY <= 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 
                                                               , CNTRIB_PREAMT  = REC_IWOL.CNTRIB_PREAMT + ( CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) )-- 전년까지 공제금액 ( 전년까지의기부금액에서  이월액을 더한다)
                                          ...
                                          (
                                              SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                               ...
                                               WHERE YY                 = IN_YY
                                                 ...
                                                 AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                 AND CNTRIB_TYPE_CD = 'A032400006'  --종교외      종교:A032400007
--eterain6 20200217 where 절에 이미포함          AND SETT_FG = 'A031300001'--연말정산인것만
                                                 AND NVL(CNTRIB_OVERAMT,0) <> 0 --전년도 결과 중 이월금액이 0 이 아닌 내역만 당년도 공제 내역에 갖고 옴
                                          )
                                          LOOP
                                                 V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_OVERAMT;
                 ...
                 BEGIN
                    SELECT SUM(NVL(A.NTS_CNTRIB_AMT,0) + NVL(A.ETC_CNTRIB_AMT,0))
                     ...
                     WHERE A.RPST_PERS_NO   = REC.RPST_PERS_NO
                       ...
                       AND A.FM_SEQ         = B.FM_SEQ
                       AND B.FM_REL_CD      = 'A034600001'  -- 정치자금은 본인만
--eterain6 20200217    --AND (B.INCOME_BELOW_YN = 'Y' OR B.FM_REL_CD = 'A034600001')--@VER.2016_4 기부금 부양가족 연령요건 삭제(소득요건만 체크 또는 본인)
                       --AND NVL(B.BASE_DUC_YN,'N') IN ('Y','1') --기본공제 체크된 사람의 기부금
                       ;
-- richardoh 소득공제의 정치자금기부금 쿼리조건과 다름  B.INCOME_BELOW_YN = 'Y' ?????
-- eterain6 20200217 해당쿼리 수정
                       EXCEPTION
                           WHEN NO_DATA_FOUND THEN
                               V_FLAW_CNTRIB_AMT := 0;
                 END;
                                      ...
                                      BEGIN 
                                              ...
                                              (
                                                  SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                   ...
                                                   WHERE YY                 = IN_YY
                                                     --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                     AND CNTRIB_YY       IN ('2014','2015') /* 14, 15년 기부금 */
                                                     AND SETT_FG          = V_SETT_FG
                                                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                     AND CNTRIB_TYPE_CD = 'A032400001'  --법정기부금
                                      ...
                                      BEGIN 
                                              ...
                                              (
                                                  SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                   ...
                                                   WHERE YY                 = IN_YY
                                                     --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                     AND CNTRIB_YY       in ('2016','2017','2018') /* 16,17,18년 기부금 */
                                                     AND SETT_FG          = V_SETT_FG
                                                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                     AND CNTRIB_TYPE_CD = 'A032400001'  --법정기부금
...
--eterain6 20200217           IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT + V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT > 20000000 THEN --직전 누적이 1천 넘을때
--eterain6 20200217                 V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 20000000)  * 0.30));         -- 1천 초과 30%
--eterain6 20200217                 V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (20000000 - (V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_DUC_OBJ_AMT) )  * 0.15));  -- 1천 이하 15%
...
_CNTRIB_DUC_OBJ_AMT,FLAW_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_DUC_OBJ_AMT,APNT_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_RELI_OUT_OBJ_AMT,APNT_CNTRIB_RELI_OUT_DUC_AMT,APNT_CNTRIB_RELI_OBJ_AMT,APNT_CNTRIB_RELI_DUC_AMT,STAD_TAXDUC_AMT,MNRT_TAXDUC_AMT,DSPSN_GUARQL_INSU_DUC_OBJ_AMT,DSPSN_GUARQL_INSU_TAXDUC_AMT,YRETXA_PART_FXD_YN,DUTY_INVENT_CMPS_AMT_NOTAX,NPN_DUC_OBJ_AMT,NPN_INSU_DUC_OBJ_AMT,PUBPERS_PENS_DUC_OBJ_AMT,MILITARY_PENS_DUC_OBJ_AMT,SPCL_DUC_HINS_DUC_OBJ_AMT,SPCL_DUC_EINS_DUC_OBJ_AMT,STOCK_BUY_CHOICE_PROFIT_AMT,SMBIZ_BONUS_TAX )
                SELECT BIZR_DEPT_CD,YRETXA_SEQ,YY,SETT_FG,RPST_PERS_NO,RPRT_YYMM,POSI_BREU_CD,POSI_DEPT_CD,KOR_NM,RES_NO,NATI_FG,RSD_FG,RSD_NATI_FG,LABOR_SCHLS_FG,STTS_FG,BIZTP_FG,WKSP_FG,LVLPT_FG,WKGD_CD,STEP_FG,CWK_YCNT,HOUSEHOLDER_YN,REDC_FR_DT,REDC_TO_DT,MA_WK_SALY_AMT,MA_WK_BONUS_AMT,MA_DETM_BONUS_AMT,MA_WK_TT_AMT,BF_WK_FIRM_NM,BF_WK_FR_DT_1,BF_WK_TO_DT_1,BF_REDC_FR_DT_1,BF_REDC_TO_DT_1,BF_WK_NO,BF_WK_SALY_AMT,BF_WK_BONUS_AMT,BF_WK_DETM_BONUS_AMT,BF_WK_REDC_TAX_AMT1,BF_WK_FIRM_2_NM,BF_WK_FR_DT_2,BF_WK_TO_DT_2,BF_WK_NO_2,BF_WK_SALY_2_AMT,BF_WK_BONUS_2_AMT,BF_WK_DETM_BONUS_2_AMT,BF_WK_REDC_TAX_AMT2,BF_WK_FIRM_3_NM,BF_WK_FR_DT_3,BF_WK_TO_DT_3,BF_REDC_FR_DT_3,BF_REDC_TO_DT_3,BF_WK_NO_3,BF_WK_SALY_3_AMT,BF_WK_BONUS_3_AMT,BF_WK_DETM_BONUS_3_AMT,BF_WK_REDC_TAX_AMT3,BF_WK_REDC_TAX_TT_AMT,BF_WK_TT_AMT,SALY_TT_AMT,BONUS_TT_AMT,DETM_BONUS_TT_AMT,TAX_TT,FRN_NOTAX_AMT,DELAY_NOTAX_AMT,RECH_NOTAX_AMT,CARE_NOTAX_AMT,LABOR_SCHLS_NOTAX_AMT,ETC_NOTAX_AMT,NOTAX_TT_AMT,REDC_EARN_AMT,LABOR_EARN_AMT,LABOR_EARN_DUC_AMT,SLF_DUC_AMT,WIFE_DUC_AMT,SPRT_PSN_CNT,SPRT_DUC,RSPT_DUC_CNT,PATH_DUC_AMT,HINDR_CNT,HIND_DUC_AMT,WOMN_DUC_AMT,BRED_CNT,BRED_DUC_AMT,CHDBIRTH_DUC_CNT,CHDBIRTH_DUC_AMT,MULT_CHILD_ADD_DUC_CNT,MULT_CHILD_ADD_DUC_AMT,SINGLE_PARENT_DUC_AMT,NPN_DUC_AMT,PUBPERS_PENS_DUC_AMT,NPN_INSU_AMT,MILITARY_PENS_DUC_AMT,RETI_SCI_PENS_EARN_DUC_AMT,RETI_PENS_EARN_DUC_AMT,SPCL_DUC_INSU_AMT,SPCL_DUC_HINS_AMT,SPCL_DUC_EINS_AMT,SPCL_DUC_GUAR_INSU_AMT,SPCL_DUC_HANDICAP_INSU_AMT,SPCL_DUC_HFE_AMT,SPCL_DUC_HANDICAP_HFE_AMT,SPCL_DUC_ETC_HFE_AMT,SPCL_DUC_ED_AMT,SPCL_DUC_HANDICAP_EDU_AMT,SPCL_DUC_ETC_EDU_AMT,SPCL_DUC_HSFND_AMT,SPCL_DUC_HOUS_LOAMT_AMT1,SPCL_DUC_HOUS_LOAMT_AMT2,SPCL_DUC_MNRT_AMT,SPCL_DUC_HOUS_ITT_AMT,SPCL_DUC_HOUS_ITT_AMT1,SPCL_DUC_HOUS_ITT_AMT2,SPCL_DUC_HOUS_ITT_AMT3,SPCL_DUC_HOUS_ITT_AMT4,SPCL_DUC_HOUS_ITT_AMT5,SPCL_DUC_HOUS_ITT_AMT6,SPCL_DUC_HOUS_ITT_AMT7,SPCL_DUC_HOUS_ITT_AMT8,SPCL_DUC_HOUS_ITT_AMT9,SPCL_DUC_CNTRIB_AMT,SPCL_DUC_MAGE_MV_AMT,STAD_DUC_AMT,SBTR_EARN_AMT,PERS_PESN_SAV_DUC_AMT,PESN_SAV_DUC_AMT,DUC_INME_EARN_DUC_AMT,SUBSCRP_SAV_DUC_AMT,HOUS_SUBSCRP_GNR_SAV_DUC_AMT,LNTM_HSSV_DUC_AMT,LABORR_HSSV_DUC_AMT,HOUS_SAV_DUC_AMT,ICOMP_DUC_AMT,DEBIT_CARD_DUC_AMT,CREDIT_CARD_DUC_AMT,OSC_EARN_DUC_AMT,LSTCS_SAV_DUC_AMT,LSTCS_SAV_DUC_AMT1,LSTCS_SAV_DUC_AMT2,LSTCS_SAV_DUC_AMT3,TXSTD_AMT,CAL_TAX,ITXLW,STXLW,RTXLW,RTXLW_OBJ_AMT,RTXLW_CURR_REDC_AMT,RTXLW_ALD_REDC_AMT,REDC_TAX_TT,LABOR_EARN_TDUC_AMT,TXPYAS_TDUC_AMT,LESE_LOAMT_TDUC_AMT,CNTRIB_POLITICS_TDUC_AMT,FRNPAI_TDUC_AMT,TDUC_TT,DETM_EARN_AMT,DETM_FMTAX_AMT,DETM_IHTAX_AMT,DETM_TT_AMT,ALD_BF_EARN_AMT,ALD_BF_FMTAX_AMT,ALD_BF_IHTAX_AMT,ALD_BF_TT_AMT,ALD_MA_EARN_AMT,ALD_MA_FMTAX_AMT,ALD_MA_IHTAX_AMT,ALD_MA_TT_AMT,PAID_SPCLEX_TAX,ALD_SBTR_EARN_AMT,ALD_SBTR_FMTAX_AMT,ALD_SBTR_IHTAX_AMT,ALD_SBTR_TT_AMT,RPRT_YN,FLAW_CNTRIB_AMT,POLITICS_TRSR_CNTRIB_AMT,POLITICS_TRSR_AMT,THYR_FLAW_CNTRIB_AMT,SPCLEX_CNTRIB_AMT,APNT_CNTRIB_AMT,RELI_GRP_APNT_CNTRIB_AMT,CNTRIB_AMT_OSC_SOCT,FLAW_CNTRIB_AMT_CYOV_AMT,SPCLEX_CNTRIB_AMT_CYOV_AMT,RELI_OUT_CNTRIB_AMT_CYOV_AMT,RELI_GRP_CNTRIB_AMT_CYOV_AMT,LFSTS_ITT_RFND_AMT,SPCL_DUC_GNR_LMT_AMT,SPCL_DUC_GNR_LMT_EXCE_AMT,CURR_WK_FR_DT,CURR_WK_TO_DT,REMK,BF_DELAY_NOTAX_AMT_1,BF_CARE_NOTAX_AMT_1,BF_ETC_NOTAX_AMT_1,BF_APNT_NOTAX_AMT_1,BF_NOTAX_TT_AMT_1,BF_DELAY_NOTAX_AMT_2,BF_CARE_NOTAX_AMT_2,BF_ETC_NOTAX_AMT_2,BF_APNT_NOTAX_AMT_2,BF_NOTAX_TT_AMT_2,BF_DELAY_NOTAX_AMT_3,BF_CARE_NOTAX_AMT_3,BF_ETC_NOTAX_AMT_3,BF_APNT_NOTAX_AMT_3,BF_NOTAX_TT_AMT_3,BF_RECH_NOTAX_AMT_1,BF_RECH_NOTAX_AMT_2,BF_RECH_NOTAX_AMT_3,FORE_TAX_RATE_YN,INPT_ID,INPT_DTTM,INPT_IP,MOD_ID,MOD_DTTM,MOD_IP,BF_REDC_FR_DT_2,BF_REDC_TO_DT_2,BASE_DUC_CHILD_CNT,CHILD_TAXDUC_AMT,CNTRIB_AMT_CYOV_AMT,OSC_CNTRB_AMT,OSC_CNTRIB_AMT,LNTM_GATHER_INVST_SEC_SAV_AMT,SCI_TECH_PSN_PENS_DUC_OBJ_AMT,SCI_TECH_PSN_PENS_TAXDUC_AMT,RETI_PENS_DUC_OBJ_AMT,RETI_PENS_TAXDUC_AMT,PNSV_DUC_OBJ_AMT,PNSV_TAXDUC_AMT,GUARQL_INSU_DUC_OBJ_AMT,GUARQL_INSU_TAXDUC_AMT,HFE_DUC_OBJ_AMT,HFE_TAXDUC_AMT,EDAMT_DUC_OBJ_AMT,EDAMT_TAXDUC_AMT,POLITICS_LMT_BLW_DUC_OBJ_AMT,POLITICS_LMT_BLW_TAXDUC_AMT,POLITICS_LMT_EXCE_DUC_OBJ_AMT,POLITICS_LMT_EXCE_TAXDUC_AMT,FLAW_CNTRIB_DUC_OBJ
