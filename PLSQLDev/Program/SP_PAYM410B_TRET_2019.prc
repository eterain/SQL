...
N_TAX_SUB_OBJ_TMP_UNDER  NUMBER := 0;    -- 기부금 누적액 이하 공제대상액 임시
N_CALC_TAX_SUB_UNDER     NUMBER := 0;    -- 기부금 누적액 이하 세액공제액 임시
N_TAX_SUB_OBJ_TMP_OVER   NUMBER := 0;    -- 기부금 누적액 이상 공제대상액 임시
N_CALC_TAX_SUB_OVER      NUMBER := 0;    -- 기부금 누적액 이상 세액공제액 임시
...
--DBMS_OUTPUT.PUT_LINE('-----신용카드최저사용금액 계산(총급여액 * 25%): ' || V_CREDIT_MINI_USE_AMT );
                  ...
                  ELSIF V_CREDIT_TOT_USE_AMT < V_CREDIT_MINI_USE_AMT AND V_CREDIT_MINI_USE_AMT <= V_CREDIT_TOT_USE_AMT + V_DEBIT_USE_AMT + V_CSH_RECPT_USE_AMT + V_CREDIT_BOOK_PFMC_AMT THEN
                    /* 신용카드사용분(전통시장,대중교통비제외) < 최저사용금액 <= 신용카드+직불카드+현금영수증+도서.공연비 사용액
                     ...
                     V_CREDIT_DUC_EXC_AMT :=  TRUNC( (V_CREDIT_TOT_USE_AMT * 15 / 100 )
                                            + ((V_CREDIT_MINI_USE_AMT - V_CREDIT_TOT_USE_AMT) * 30 / 100) )  ;

--DBMS_OUTPUT.PUT_LINE('2-----신용카드사용분*15% + {최저사용금액 - 신용카드사용분}*30% ' || TRUNC(V_CREDIT_TOT_USE_AMT * 15 / 100 ) || ' , ' || TRUNC((V_CREDIT_MINI_USE_AMT - V_CREDIT_TOT_USE_AMT) * 30 / 100) );
--DBMS_OUTPUT.PUT_LINE('2 edit 신용카드사용분*15% + {최저사용금액 - 신용카드사용분}*30% ' || TRUNC( ( V_CREDIT_TOT_USE_AMT * 15 / 100 ) + ( (V_CREDIT_MINI_USE_AMT - V_CREDIT_TOT_USE_AMT) * 30 / 100 ) ) );
                  ELSIF V_CREDIT_MINI_USE_AMT > V_CREDIT_TOT_USE_AMT + V_DEBIT_USE_AMT + V_CSH_RECPT_USE_AMT + V_CREDIT_BOOK_PFMC_AMT THEN
                    /* 최저사용금액 > 신용카드+직불카드+현금영수증 사용액  : 신용카드*15% + (직불카드+현금영수증+도서.공연비)*30% + {최저사용금액-(신용카드+직불카드+현금영수증+도서공연비)}* 40% */
                     V_CREDIT_DUC_EXC_AMT :=  TRUNC(( V_CREDIT_TOT_USE_AMT * 15 / 100 )
                                            + ((V_DEBIT_USE_AMT + V_CSH_RECPT_USE_AMT+ V_CREDIT_BOOK_PFMC_AMT) * 30 / 100)
                                            + ((V_CREDIT_MINI_USE_AMT - (V_CREDIT_TOT_USE_AMT+V_DEBIT_USE_AMT+V_CSH_RECPT_USE_AMT+V_CREDIT_BOOK_PFMC_AMT)) * 40 / 100)) ;
...
--DBMS_OUTPUT.PUT_LINE('-----도서.공연비 사용액(총급여 7천만원 이하자) : ' || V_CREDIT_DUC_POSS_AMT );
...
--            RAISE USER_DEFINE_ERROR;
        ...
        SELECT NVL(SUM(CASE WHEN CNTRIB_TYPE_CD = 'A032400001' AND CNTRIB_YY = '2019' THEN NVL(CNTRIB_GONGAMT, 0) ELSE 0 END), 0) AS  GN_RT_TOTAL_CUR_SUB     -- 당해년도 법정 기부금                ,
           ...
           ***************************************************************************************************************************/
            ...
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_TOTAL_ETC_SUB_14), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_ETC_SUB_14 - 30000000, 0), GN_RT_TOTAL_ETC_SUB_14); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            
            
            IF GV_TAX_REMAIN_AMT < N_CALC_TAX_SUB_UNDER THEN           -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)     
             
                N_RE_CALC_TAX_OBJ        := CEIL(GV_TAX_REMAIN_AMT / 15 * 100); 
                N_CALC_TAX_SUB           := GV_TAX_REMAIN_AMT;

            ELSIF GV_TAX_REMAIN_AMT < N_CALC_TAX_SUB_UNDER + N_CALC_TAX_SUB_OVER THEN           -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)     

                N_RE_CALC_TAX_OBJ        := N_TAX_SUB_OBJ_TMP_UNDER + CEIL((GV_TAX_REMAIN_AMT - N_CALC_TAX_SUB_UNDER) / 25 * 100); 
                N_CALC_TAX_SUB           := GV_TAX_REMAIN_AMT;
            ELSE
                N_RE_CALC_TAX_OBJ        := N_TAX_SUB_OBJ_TMP_UNDER + N_TAX_SUB_OBJ_TMP_OVER; 
                N_CALC_TAX_SUB           := FLOOR(N_CALC_TAX_SUB_UNDER + N_CALC_TAX_SUB_OVER);                                                      -- 계산된 기부금 세액공제액 임시 저장
...
--            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP * 15 / 100;                                 --  세액공제액(20200219)
--
--            
--            IF GV_TAX_REMAIN_AMT < N_TAX_SUB_TMP THEN           -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)     
--
...
--                GV_TAX_REMAIN_AMT           := GV_TAX_REMAIN_AMT - N_CALC_TAX_SUB;                                                        --  잔여세액에서 차감
--                
--            END IF;                                             -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)  
--            
--            --  3000 만원 초과 25%
...
--            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP * 25 / 100;                                --  세액공제액(20200219)
--            
--            
--            IF GV_TAX_REMAIN_AMT < N_TAX_SUB_TMP THEN           -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)  
--
...
--            GN_RT_TOTAL_ETC_SUB_14      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_TOTAL_ETC_SUB_15), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_ETC_SUB_15 - 30000000, 0), GN_RT_TOTAL_ETC_SUB_15); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            ...
            GN_RT_TOTAL_ETC_SUB_15      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_TOTAL_ETC_SUB_16), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_ETC_SUB_16 - 20000000, 0), GN_RT_TOTAL_ETC_SUB_16); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            ELSIF GV_TAX_REMAIN_AMT < N_CALC_TAX_SUB_UNDER + N_CALC_TAX_SUB_OVER THEN           -- 잔여세액보다 계산된 세액이 크다면 잔여세액만큼만 처리(20200219 DY)     
                ...
                N_RE_CALC_TAX_OBJ        := N_TAX_SUB_OBJ_TMP_UNDER + CEIL((GV_TAX_REMAIN_AMT - N_CALC_TAX_SUB_UNDER) / 30 * 100); 
                N_CALC_TAX_SUB           := GV_TAX_REMAIN_AMT;
            ELSE
                N_RE_CALC_TAX_OBJ        := N_TAX_SUB_OBJ_TMP_UNDER + N_TAX_SUB_OBJ_TMP_OVER; 
                N_CALC_TAX_SUB           := FLOOR(N_CALC_TAX_SUB_UNDER + N_CALC_TAX_SUB_OVER);                                                      -- 계산된 기부금 세액공제액 임시 저장
            ...
            GN_RT_TOTAL_ETC_SUB_16      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_TOTAL_ETC_SUB_17), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_ETC_SUB_17 - 20000000, 0), GN_RT_TOTAL_ETC_SUB_17); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            GN_RT_TOTAL_ETC_SUB_17      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_TOTAL_ETC_SUB_18), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_ETC_SUB_18 - 20000000, 0), GN_RT_TOTAL_ETC_SUB_18); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            GN_RT_TOTAL_ETC_SUB_18      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  1000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(10000000 - N_CMLTV_GIFT, GN_RT_TOTAL_CUR_SUB), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  1000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_TOTAL_CUR_SUB - 10000000, 0), GN_RT_TOTAL_CUR_SUB); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            GN_RT_TOTAL_CUR_SUB      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
           ...
           ***************************************************************************************************************************/
            ...
            --  1000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(10000000 - N_CMLTV_GIFT, GN_STOCK_URSM), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  1000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_STOCK_URSM - 10000000, 0), GN_STOCK_URSM); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            GN_STOCK_URSM      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
...
--            GN_STOCK_URSM               := N_RE_CALC_TAX_OBJ;
           ...
           ***************************************************************************************************************************/
            ...
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_APPNT_14), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_APPNT_14 - 30000000, 0), GN_RT_PSA_ETC_APPNT_14); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            ...
            GN_RT_PSA_ETC_APPNT_14      := N_RE_CALC_TAX_OBJ;                                                                            -- 당해 처리된 공제액 
            ...
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_APPNT_15), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_APPNT_15 - 30000000, 0), GN_RT_PSA_ETC_APPNT_15); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            ...
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_APPNT_16), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_APPNT_16 - 20000000, 0), GN_RT_PSA_ETC_APPNT_16); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_APPNT_17), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_APPNT_17 - 20000000, 0), GN_RT_PSA_ETC_APPNT_17); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체외 지정기부금 (2018년) ******************/
            --  2000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_APPNT_18), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_APPNT_18 - 20000000, 0), GN_RT_PSA_ETC_APPNT_18); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체외 지정기부금 (당해분) ******************/
            --  1000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(10000000 - N_CMLTV_GIFT, GN_RT_PSA_CUR_APPNT), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  1000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_CUR_APPNT - 10000000, 0), GN_RT_PSA_CUR_APPNT); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
           ...
           ***************************************************************************************************************************/
            ...
            /*************** 종교단체 지정기부금 (2014년) ********************/
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_RELGN_14), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_RELGN_14 - 30000000, 0), GN_RT_PSA_ETC_RELGN_14); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체 지정기부금 (2015년) ********************/
            --  3000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(30000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_RELGN_15), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  3000 만원 초과 25%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_RELGN_15 - 30000000, 0), GN_RT_PSA_ETC_RELGN_15); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 25 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체 지정기부금 (2016년) ********************/
            --  23000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_RELGN_16), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_RELGN_16 - 20000000, 0), GN_RT_PSA_ETC_RELGN_16); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체 지정기부금 (2017년) ********************/
            --  23000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_RELGN_17), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_RELGN_17 - 20000000, 0), GN_RT_PSA_ETC_RELGN_17); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체 지정기부금 (2018년) ********************/
            --  23000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(20000000 - N_CMLTV_GIFT, GN_RT_PSA_ETC_RELGN_18), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  2000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_ETC_RELGN_18 - 20000000, 0), GN_RT_PSA_ETC_RELGN_18); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
            ...
            /*************** 종교단체 지정기부금 (당해분) ********************/
            --  1000 만원 이하 15%
            N_TAX_SUB_OBJ_TMP_UNDER     := GREATEST(LEAST(10000000 - N_CMLTV_GIFT, GN_RT_PSA_CUR_RELGN), 0); --  세액공제 대상액
            N_CALC_TAX_SUB_UNDER        := N_TAX_SUB_OBJ_TMP_UNDER * 15 / 100;                                 --  세액공제액(20200219)
            --  1000 만원 초과 30%
            N_TAX_SUB_OBJ_TMP_OVER      := LEAST(GREATEST(N_CMLTV_GIFT + GN_RT_PSA_CUR_RELGN - 10000000, 0), GN_RT_PSA_CUR_RELGN); --  세액공제 대상액
            N_CALC_TAX_SUB_OVER         := N_TAX_SUB_OBJ_TMP_OVER * 30 / 100;                                --  세액공제액(20200219)
...
--            V_PAYM452_YY                            := '';
            ...
            N_TAX_SUB_OBJ_TMP_UNDER                 := 0;    -- 기부금 누적액 이하 공제대상액 임시
            N_CALC_TAX_SUB_UNDER                    := 0;    -- 기부금 누적액 이하 세액공제액 임시
            N_TAX_SUB_OBJ_TMP_OVER                  := 0;    -- 기부금 누적액 이상 공제대상액 임시
            N_CALC_TAX_SUB_OVER                     := 0;    -- 기부금 누적액 이상 세액공제액 임시
