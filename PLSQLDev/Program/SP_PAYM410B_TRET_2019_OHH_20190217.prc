CREATE OR REPLACE PROCEDURE SP_PAYM410B_TRET_2019_OHH
(
...
--2019 소득공제에서 각 한도계산 -> 세액공제에서 재사용 
--상단에 V_LMT_CNTRIB_AMT 기부금공제한도 있음 
V_LMT_CNTRIB_20_AMT                     NUMBER(15) := 0;      --기부금공제대상금액 한도 정치
V_LMT_CNTRIB_10_AMT                     NUMBER(15) := 0;      --기부금공제대상금액 한도 법정
V_LMT_CNTRIB_4041_AMT                   NUMBER(15) := 0;      --기부금공제대상금액 한도 지정

--2019 기부유형별 고액기부금처리기준 연도별 합

V_CNTRIB_10_2013_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 법정 2013
V_CNTRIB_10_PPRE_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 법정 2014,15
V_CNTRIB_10_PRE_SUM_AMT                 NUMBER(15) := 0;      --기부금입력 합 법정 2016,17,18
V_CNTRIB_10_YEAR_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 법정 당년

V_CNTRIB_40_2013_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교외 2013
V_CNTRIB_40_PPRE_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교외 2014,15
V_CNTRIB_40_PRE_SUM_AMT                 NUMBER(15) := 0;      --기부금입력 합 종교외 2016,17,18
V_CNTRIB_40_YEAR_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교외 당년

V_CNTRIB_41_2013_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교 2013
V_CNTRIB_41_PPRE_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교 2014,15
V_CNTRIB_41_PRE_SUM_AMT                 NUMBER(15) := 0;      --기부금입력 합 종교 2016,17,18
V_CNTRIB_41_YEAR_SUM_AMT                NUMBER(15) := 0;      --기부금입력 합 종교 당년

--2019 고액기부금 기준이하 및 기준초과 처리용
V_CNTRIB_UP_AMT                         NUMBER(15) := 0;      --기부금 고액초과금액
V_CNTRIB_DOWN_AMT                    NUMBER(15) := 0;      --기부금 고액이하금액
V_LMT_CNTRIB_SUM_AMT                 NUMBER(15) := 0;      --기부금 누적 공제 대상 금액 

/*
상단의 변수 참조하려고 여기 주석으로 기입 
V_FLAW_CNTRIB_DUC_OBJ_AMT               NUMBER(15) := 0;      --법정기부공제대상금액
V_FLAW_CNTRIB_TAXDUC_AMT                NUMBER(15) := 0;      --법정기부세액공제액
V_APNT_CNTRIB_DUC_OBJ_AMT               NUMBER(15) := 0;      --지정기부공제대상금액
V_APNT_CNTRIB_TAXDUC_AMT                NUMBER(15) := 0;      --지정기부세액공제액
...
V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT              NUMBER(15) := 0;      --법정기부공제대상금액 14,15
V_FLAW_CNTRIB_PPRE_TAXDUC_AMT               NUMBER(15) := 0;      --법정기부세액공제액 14,15
V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT               NUMBER(15) := 0;      --법정기부공제대상금액 16~18
V_FLAW_CNTRIB_PRE_TAXDUC_AMT                NUMBER(15) := 0;      --법정기부세액공제액 16~18

V_CNTRIB_40_PPRE_DUC_OBJ_AMT            NUMBER(15) := 0;      --지정기부(종교외)공제대상금액 14,15
V_CNTRIB_40_PPRE_TAXDUC_AMT             NUMBER(15) := 0;      --지정기부(종교외)세액공제액 14,15
V_CNTRIB_40_PRE_DUC_OBJ_AMT             NUMBER(15) := 0;      --지정기부(종교외)공제대상금액 16~18
V_CNTRIB_40_PRE_TAXDUC_AMT              NUMBER(15) := 0;      --지정기부(종교외)세액공제액 16~18

V_CNTRIB_41_PPRE_DUC_OBJ_AMT            NUMBER(15) := 0;      --지정기부(종교)공제대상금액 14,15
V_CNTRIB_41_PPRE_TAXDUC_AMT             NUMBER(15) := 0;      --지정기부(종교)세액공제액 14,15
V_CNTRIB_41_PRE_DUC_OBJ_AMT             NUMBER(15) := 0;      --지정기부(종교)공제대상금액 16~18
V_CNTRIB_41_PRE_TAXDUC_AMT              NUMBER(15) := 0;      --지정기부(종교)세액공제액 16~18

/*
-- 프로시저 적용시   
SELECT SF_PAYM410_CNTRIB_AMT_2019_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '7_20161718') AS AMT
  FROM DUAL

IN_BIZR_DEPT_CD       IN     VARCHAR2,     --사업자부서코드
    IN_YY                 IN     VARCHAR2,     --년도
    IN_YRETXA_SEQ         IN     VARCHAR2,     --정산차수
    IN_SETT_FG            IN     VARCHAR2,     --정산구분
    IN_RPST_PERS_NO       IN     VARCHAR2,     --대표개인번호 
    IN_BIZR_REG_NO        IN     VARCHAR2,     --사업자번호
    IN_CNTRIB_TYPE_YERAR  IN     VARCHAR2      --기부유형 및 년도구간(기부유형 1자리 + 년도구간 )
                                               --1_2013 : 법정_2013년도
                                               --1_201415 : 법정_2014, 2015년도
                                               --1_20161718 : 법정_2016,2017, 2018년도
                                               --1_2019 : 법정_2019년도
                                               --6_2013 : 지정_2013년도 ..
                                               --7_2013 : 종교_2013년도 ..
*/
...
--            END IF;
            ...
            IF IN_YY = '2019' AND IN_BIZR_DEPT_CD = '00000' AND IN_YRETXA_SEQ = '1' AND IN_SETT_FG = 'A031300001' AND REC.RPST_PERS_NO = 'A072436' THEN   -- 진영원 예외 처리 . 종근무지 직무발명보상금 추가
                V_LABOR_EARN_TT_SALY_AMT := V_CURR_SITE_SALY_AMT + V_CURR_SITE_BONUS_AMT + V_CURR_SITE_DETM_BONUS_AMT +
                                            V_BF_SITE_SALY_AMT + V_BF_SITE_BONUS_AMT + V_BF_SITE_DETM_BONUS_AMT + V_BF_OSC_SOCT_WITHD_AMT + V_BF_STOCK_BUY_AMT + 970745;
            ELSE
                V_LABOR_EARN_TT_SALY_AMT := V_CURR_SITE_SALY_AMT + V_CURR_SITE_BONUS_AMT + V_CURR_SITE_DETM_BONUS_AMT +
...
/** 기부금공제 시작 **/
            -- 법정, 지정(종교, 종교외) 작년 이월 기부금, 당해년도 기부금, 당년도 급여 노조비 기부금 테이블에 INSERT. @@yoo 
            -- 당해년도의 경우 공제금액이 이월금액에도 동일하게 들어가면 되는지?
            IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                ...
                SELECT IN_BIZR_DEPT_CD
                  ...
                  FROM (
                        SELECT A1.RPST_PERS_NO, A1.CNTRIB_YY, A1.CNTRIB_TYPE_CD
                          ...
                          FROM (
                                SELECT /*+ LEADING(a) USE_NL(b) INDEX(a IDX_PAYM432_01) */        --@TUNING
                                 ...
                                 WHERE A.RPST_PERS_NO = REC.RPST_PERS_NO
                                   ...
                                   AND A.CNTRIB_TYPE_CD IN ('A032400001', 'A032400006', 'A032400007')  --법정기부금, 지정기부금, 종교기부금 
                          ...
                          GROUP BY A1.RPST_PERS_NO, A1.CNTRIB_YY, A1.CNTRIB_TYPE_CD
                       ) C1;
            ELSE
...
/*@@eterain*/
/*V_OCCR_LOC_NM := '기부금2019년변수확인-1';
V_DB_ERROR_CTNT := '근로소득금액-V_LABOR_EARN_AMT:'||V_LABOR_EARN_AMT||chr(13)||chr(10)||
                   '특별소득공제-V_SPCL_DUC_AMT:'||V_SPCL_DUC_AMT||chr(13)||chr(10)||
                   '표준세액공제-V_STAD_TAXDUC_OBJ_AMT:'||V_STAD_TAXDUC_OBJ_AMT||chr(13)||chr(10)||
                   '산출세액(합산)-V_TDUC_DUC_TT_AMT:'||V_TDUC_DUC_TT_AMT||chr(13)||chr(10)||
                   '산출세액(차감)V_CAL_TDUC_TEMP_AMT:'||V_CAL_TDUC_TEMP_AMT||chr(13)||chr(10)
                   ;
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */


-- richardoh 기부금 소득공제 13년도 이월분 시작
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '1_2013'    ) INTO V_CNTRIB_10_2013_SUM_AMT FROM DUAL; --기부금입력 합 법정 2013        
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '1_201415'  ) INTO V_CNTRIB_10_PPRE_SUM_AMT FROM DUAL; --기부금입력 합 법정 2014,15     
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '1_20161718') INTO V_CNTRIB_10_PRE_SUM_AMT  FROM DUAL; --기부금입력 합 법정 2016,17,18  
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '1_2019'    ) INTO V_CNTRIB_10_YEAR_SUM_AMT FROM DUAL; --기부금입력 합 법정 당년         
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '6_2013'    ) INTO V_CNTRIB_40_2013_SUM_AMT FROM DUAL; --기부금입력 합 종교외 2013      
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '6_201415'  ) INTO V_CNTRIB_40_PPRE_SUM_AMT FROM DUAL; --기부금입력 합 종교외 2014,15   
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '6_20161718') INTO V_CNTRIB_40_PRE_SUM_AMT  FROM DUAL; --기부금입력 합 종교외 2016,17,18
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '6_2019'    ) INTO V_CNTRIB_40_YEAR_SUM_AMT FROM DUAL; --기부금입력 합 종교외 당년        
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '7_2013'    ) INTO V_CNTRIB_41_2013_SUM_AMT FROM DUAL; --기부금입력 합 종교 2013        
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '7_201415'  ) INTO V_CNTRIB_41_PPRE_SUM_AMT FROM DUAL; --기부금입력 합 종교 2014,15     
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '7_20161718') INTO V_CNTRIB_41_PRE_SUM_AMT  FROM DUAL; --기부금입력 합 종교 2016,17,18  
                SELECT SF_PAYM410_CNTRIB_AMT_2019(IN_BIZR_DEPT_CD, IN_YY, IN_YRETXA_SEQ, V_SETT_FG , REC.RPST_PERS_NO, V_BIZR_REG_NO, '7_2019'    ) INTO V_CNTRIB_41_YEAR_SUM_AMT FROM DUAL; --기부금입력 합 종교 당년         
 
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 법정 2013  : ' || V_CNTRIB_10_2013_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 법정 2014,15  : ' || V_CNTRIB_10_PPRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 법정 2016,17,18  : ' || V_CNTRIB_10_PRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 법정 당년  : ' || V_CNTRIB_10_YEAR_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교외 2013  : ' || V_CNTRIB_40_2013_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교외 2014,15  : ' || V_CNTRIB_40_PPRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교외 2016,17,18  : ' || V_CNTRIB_40_PRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교외 당년  : ' || V_CNTRIB_40_YEAR_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교 2013  : ' || V_CNTRIB_41_2013_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교 2014,15  : ' || V_CNTRIB_41_PPRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교 2016,17,18  : ' || V_CNTRIB_41_PRE_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@기부금입력 합 종교 당년  : ' || V_CNTRIB_41_YEAR_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            ----------------------------------------------------------');
                DBMS_OUTPUT.PUT_LINE('.            @@근로소득금액 V_LABOR_EARN_AMT : ' || V_LABOR_EARN_AMT);
                DBMS_OUTPUT.PUT_LINE('.            ----------------------------------------------------------');
            /** 기부금 소득공제  13년도 이월분  **/
-- richardoh 13년도 이월기부금 공제한도 시작

               --기부금공제대상금액 한도 정치  근로소득금액 * 100% 일단 갖고있음 --세액공제에서 필요시 사용
                V_LMT_CNTRIB_20_AMT :=V_LABOR_EARN_AMT; 
                --기부금공제대상금액      
                V_LMT_CNTRIB_AMT := V_LMT_CNTRIB_20_AMT; 
                
                /** 1.1. 정치자금기부금(20) 공제대상금액 한도내에서 전액 소득 공제, 이월 없음**/
                 BEGIN
                    SELECT NVL(SUM(NVL(A.NTS_CNTRIB_AMT,0) + NVL(A.ETC_CNTRIB_AMT,0)),0)
                      INTO V_FLAW_CNTRIB_AMT
                      FROM PAYM423 A
                             , PAYM421 B --당년도 등록 공제 내역
                     WHERE A.RPST_PERS_NO   = REC.RPST_PERS_NO
                       AND A.YY             = IN_YY
                       AND A.YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                       ...
                       ;
-- richardoh 세액공제의 정치자금기부금 쿼리조건과 다름  B.INCOME_BELOW_YN = 'Y' ????
                       EXCEPTION
                           WHEN NO_DATA_FOUND THEN
                               V_FLAW_CNTRIB_AMT := 0;
                 END;

                DBMS_OUTPUT.PUT_LINE('.            @@정치자금기부금        입력금액  : ' || V_FLAW_CNTRIB_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@정치자금기부금        한도금액  : ' || V_LMT_CNTRIB_20_AMT);

---- richardoh 정치 소득공제 대상금액한도 : 근로소득금액 * 100%   한도 내에서 전액 소득공제....
                IF NVL(V_FLAW_CNTRIB_AMT,0) > V_LMT_CNTRIB_AMT THEN
                   V_FLAW_CNTRIB_AMT := V_LMT_CNTRIB_AMT;
                END IF;

                DBMS_OUTPUT.PUT_LINE('.            @@정치자금기부금        입력금액 한도처리  : ' || V_FLAW_CNTRIB_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@정치자금기부금        한도금액             : ' || V_LMT_CNTRIB_AMT);

                --기부금공제대상금액 한도 법정 (근로소득금액  - 정치자금기부금 공제 대상한도내의 입력 금액) * 100%
                V_LMT_CNTRIB_10_AMT  := GREATEST(0, V_LABOR_EARN_AMT - NVL(V_FLAW_CNTRIB_AMT,0) ); 
DBMS_OUTPUT.PUT_LINE('.            eterain 법정기부금        한도금액        V_LMT_CNTRIB_10_AMT       : ' || V_LMT_CNTRIB_10_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain 법정기부금        근로소득금액       V_LABOR_EARN_AMT       : ' || V_LABOR_EARN_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain 법정기부금        정치공제대상금액  V_FLAW_CNTRIB_AMT       : ' || V_FLAW_CNTRIB_AMT);
                --기부금공제대상 한도 금액
                --V_LMT_CNTRIB_AMT      := V_LMT_CNTRIB_10_AMT;

                 --공제 대상금액  입력금액과 대상금액한도 비교  
                V_CNTRIB_DUC_AMT     := LEAST(  V_CNTRIB_10_2013_SUM_AMT   , V_LMT_CNTRIB_10_AMT );
                V_CNTRIB_DUC_AMT    := GREATEST(0, V_CNTRIB_DUC_AMT);                                         -- 공제금액-소득공제

                DBMS_OUTPUT.PUT_LINE('.            @@법정기년부금 13년        입력금액  : ' || V_CNTRIB_10_2013_SUM_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@법정기년부금 13년        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@법정기년부금 13년   공제대상금액  : ' || V_CNTRIB_DUC_AMT);
                DBMS_OUTPUT.PUT_LINE('.            @@법정기년부금 13년        공제금액  : ' || V_CNTRIB_DUC_AMT);
                ...
                SELECT SF_SETT_CHAGAM_CAL(V_LABOR_TEMP_AMT,V_CNTRIB_DUC_AMT,1),
                  ...
                  FROM DUAL;
                DBMS_OUTPUT.PUT_LINE('.            @@법정기년부금 13년        공제금액 - 차감소득처리  : ' || V_CNTRIB_DUC_AMT);
                ...
                -- 기부금 이월분(소득공제만) 누적합 
                V_CNTRIB_AMT_CYOV_AMT :=V_CNTRIB_AMT_CYOV_AMT + V_CNTRIB_DUC_AMT; -- 기부금(이월분). @VER.2019_11

                --지정기부금 소득공제대상금액 한도 구하려고..  선처리...
                V_LMT_CNTRIB_AMT     := GREATEST( 0 , V_LABOR_EARN_AMT - V_FLAW_CNTRIB_AMT - (V_CNTRIB_10_2013_SUM_AMT + V_CNTRIB_10_PPRE_SUM_AMT + V_CNTRIB_10_PRE_SUM_AMT + V_CNTRIB_10_YEAR_SUM_AMT));

                -- 이월금 처리 시작
                IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우

                        IF V_CNTRIB_DUC_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                              BEGIN 
                                      V_TMP_CNT := 0;
                                      V_TMP_AMT := V_CNTRIB_DUC_AMT; --(35)기부금(이월분)
                                      FOR REC_IWOL IN
                                      (
                                          SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                   NVL(CNTRIB_PREAMT,0) CNTRIB_PREAMT,   --전년까지 공제금액
                                                   NVL(CNTRIB_GONGAMT,0) CNTRIB_GONGAMT,  --당년 공제금액
                                                   NVL(CNTRIB_DESTAMT,0) CNTRIB_DESTAMT,  --당년 소멸금액
                                                   CNTRIB_GIAMT, --기부금액 
                                                   CNTRIB_TYPE_CD,
                                                   CNTRIB_YY
                                            FROM PAYM436
                                           WHERE YY                 = IN_YY
                                             --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                             AND CNTRIB_YY       = '2013' /* 2013년 이전 이월 기부금 */
                                             AND SETT_FG          = V_SETT_FG
                                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                             AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                             AND CNTRIB_TYPE_CD = 'A032400001'  --법정기부금
                                             --AND SETT_FG = 'A031300001'--연말정산인것만
                                             AND NVL(CNTRIB_OVERAMT,0) <> 0 --전년도 결과 중 이월금액이 0 이 아닌 내역만 당년도 공제 내역에 갖고 옴
                                      )
                                      LOOP
                                             V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_OVERAMT;
                                             IF V_TMP_AMT >= 0 THEN
                                                  UPDATE PAYM436
                                                     SET CNTRIB_OVERAMT = 0 --당년이월금액 
                                                       , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
                                                       , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                       , CNTRIB_DESTAMT = 0
                                                   WHERE YY             = IN_YY
                                                     --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                     AND CNTRIB_YY      = REC_IWOL.CNTRIB_YY
                                                     AND SETT_FG        = V_SETT_FG
                                                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                     AND CNTRIB_TYPE_CD = REC_IWOL.CNTRIB_TYPE_CD
                                                     ;
                                             ELSE
                                                    IF V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT > 0 THEN
                                                         -- 당년 소멸금액 법이 어캐 바뀔지 모르니 2022년에..바꿔도 될듯   
                                                          UPDATE PAYM436
                                                             SET CNTRIB_OVERAMT = CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT)  --당년이월금액  CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY <= 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 
                                                               , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
                                                               , CNTRIB_GONGAMT = V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                                 --당년소멸금액
                                                               --, CNTRIB_DESTAMT = CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY > 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 
                                                           WHERE YY             = IN_YY
                                                             --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                             AND CNTRIB_YY      = REC_IWOL.CNTRIB_YY
                                                             AND SETT_FG        = V_SETT_FG
     ...
     -- 종교단체 기부금이 없는경우
               ...
               IF V_CNTRIB_41_2013_SUM_AMT + V_CNTRIB_41_PPRE_SUM_AMT + V_CNTRIB_41_PRE_SUM_AMT + V_CNTRIB_41_YEAR_SUM_AMT  < 1 THEN
                    DBMS_OUTPUT.PUT_LINE('.         ####지정기부금 (종교단체외 ) 종교단체 기부금이 없는경우 ');
                    --기부금공제대상금액 한도 지정 (근로소득금액  - 정치-법정 )*0.3
                    V_LMT_CNTRIB_4041_AMT := GREATEST(0, FLOOR( V_LMT_CNTRIB_AMT * 0.3 ) );

                    --공제 대상금액  입력금액과 대상금액한도 비교  
                    V_CNTRIB_DUC_AMT     := LEAST(  V_CNTRIB_40_2013_SUM_AMT   , V_LMT_CNTRIB_4041_AMT );
                    V_CNTRIB_DUC_AMT    := GREATEST(0, V_CNTRIB_DUC_AMT);                                         -- 공제금액-소득공제

                    DBMS_OUTPUT.PUT_LINE('.            @@종교단체외 13년        입력금액  : ' || V_CNTRIB_40_2013_SUM_AMT);
                    DBMS_OUTPUT.PUT_LINE('.            @@종교단체외 13년   공제대상금액  : ' || V_CNTRIB_DUC_AMT);
                    DBMS_OUTPUT.PUT_LINE('.            @@종교단체외 13년        공제금액  : ' || V_LMT_CNTRIB_4041_AMT);
                    DBMS_OUTPUT.PUT_LINE('.            @@종교단체외 13년        한도금액  : ' || V_CNTRIB_DUC_AMT);
                    ...
                    SELECT SF_SETT_CHAGAM_CAL(V_LABOR_TEMP_AMT,V_CNTRIB_DUC_AMT,1),
                      ...
                      INTO V_LABOR_TEMP_AMT, V_CNTRIB_DUC_AMT
                      FROM DUAL;
                    DBMS_OUTPUT.PUT_LINE('.            @@종교단체외 13년        공제금액 - 차감소득처리  : ' || V_CNTRIB_DUC_AMT);
                                          ...
                                          (
                                              SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                               ...
                                               WHERE YY                 = IN_YY
                                                 ...
                                                 AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                 AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                 AND CNTRIB_TYPE_CD = 'A032400006'  --종교외      종교:A032400007
                                                 --AND SETT_FG = 'A031300001'--연말정산인것만
                                                 AND NVL(CNTRIB_OVERAMT,0) <> 0 --전년도 결과 중 이월금액이 0 이 아닌 내역만 당년도 공제 내역에 갖고 옴
                                          )
                                          LOOP
               ...
               ELSE
                    DBMS_OUTPUT.PUT_LINE('.         ####지정기부금 (종교단체 ) 종교단체 기부금이 있는경우 ');
                    --기부금공제대상금액 한도 지정 (근로소득금액  - 정치-법정 )*0.3
                    V_LMT_CNTRIB_4041_AMT := GREATEST(0,   FLOOR(  V_LMT_CNTRIB_AMT * 0.1  ) )
                                           + LEAST ( GREATEST(0,   FLOOR(  V_LMT_CNTRIB_AMT * 0.2  ) ) , GREATEST(0, (V_CNTRIB_41_2013_SUM_AMT + V_CNTRIB_41_PPRE_SUM_AMT + V_CNTRIB_41_PRE_SUM_AMT + V_CNTRIB_41_YEAR_SUM_AMT) ) );
...
--insert into tmp_msg values (REC.RPST_PERS_NO ||', V_SPCL_INCMDED_TT_AMT : ' ||V_SPCL_INCMDED_TT_AMT||
--                            ', V_SBTR_EARN_AMT : ' ||V_SBTR_EARN_AMT );
                 ...
                 BEGIN
                    SELECT NVL(SUM(NVL(A.NTS_CNTRIB_AMT,0) + NVL(A.ETC_CNTRIB_AMT,0)),0)
                     ...
                     --AND NVL(B.BASE_DUC_YN,'N') IN ('Y','1') --기본공제 체크된 사람의 기부금
                       ;
-- richardoh 소득공제의 정치자금기부금 쿼리조건과 다름  B.INCOME_BELOW_YN = 'Y' ?????
                       EXCEPTION
                           WHEN NO_DATA_FOUND THEN
                               V_FLAW_CNTRIB_AMT := 0;
                 END;
                 ...
                 IF V_FLAW_CNTRIB_AMT <> 0 THEN
                   BEGIN
                     ...
                     DBMS_OUTPUT.PUT_LINE('.        정치기부입력금액              : '|| V_FLAW_CNTRIB_AMT );
                     DBMS_OUTPUT.PUT_LINE('.        정치기부공제대상금액 10이하   : '|| V_POLITICS_BLW_DUC_OBJ_AMT );
                     DBMS_OUTPUT.PUT_LINE('.        정치기부공제대상금액 10초과   : '|| V_POLITICS_EXCE_DUC_OBJ_AMT );
                     DBMS_OUTPUT.PUT_LINE('.        정치기부공제금액 10이하       : '|| V_POLITICS_BLW_TAXDUC_AMT );
                     DBMS_OUTPUT.PUT_LINE('.        정치기부공제금액 10초과       : '|| V_POLITICS_EXCE_TAXDUC_AMT );
                   ...
                   END;
                 END IF;
-- richardoh 기부금 세액공제 시작 정치 종료 


-- richardoh 기부금 세액공제 시작 법정 시작    
             -- 14,15 이월금액 
                 IF V_CNTRIB_10_PPRE_SUM_AMT > 0 THEN    --입력금액 
                    --법정공제대상금액한도 V_LMT_CNTRIB_10_AMT
                    V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT ); --한도금액 
                    V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_PPRE_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제대상금액
                      IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                              IF V_LMT_CNTRIB_SUM_AMT > 30000000 THEN --누적이 3000만원을 초과하면
                                    V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 30000000)  * 0.25));         -- 3천 초과 25%
                                    V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (30000000 - (V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT) )  * 0.15));  -- 3천 이하 15%
                              ELSE                                                     --누적이 3천 안될경우
                                    V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT  * 0.15));                                 -- 3천 이하 15%
                              END IF;

                          V_FLAW_CNTRIB_PPRE_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;             --공제금액 
                      END IF;

                        DBMS_OUTPUT.PUT_LINE('.            @@법정  14,15        입력금액  : ' || V_CNTRIB_10_PPRE_SUM_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  14,15        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  14,15  공제대상금액  : ' || V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  14,15         공제금액  : ' || V_FLAW_CNTRIB_PPRE_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액                       한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                        3천이하  : ' || V_CNTRIB_DOWN_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                        3천초과  : ' || V_CNTRIB_UP_AMT);

                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PPRE_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PPRE_TAXDUC_AMT);
                          ELSE
                            V_FLAW_CNTRIB_PPRE_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                            IF V_FLAW_CNTRIB_PPRE_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                            ELSE
                                V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_FLAW_CNTRIB_PPRE_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.25);
                            END IF;
                    ...
                    --################################################################
                        ...
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  14,15 공제대상금액  : ' || V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  14,15      공제금액  : ' || V_FLAW_CNTRIB_PPRE_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리                        한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
 

                        -- 이월금 처리 시작
                        IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우

                                IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                      BEGIN 
                                              V_TMP_CNT := 0;
                                              V_TMP_AMT := V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT; --(35)기부금(이월분)
                                              FOR REC_IWOL IN
                                              (
                                                  SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                           NVL(CNTRIB_PREAMT,0) CNTRIB_PREAMT,   --전년까지 공제금액
                                                   ...
                                                   WHERE YY                 = IN_YY
                                                     --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                     AND CNTRIB_YY       in ('2014','2015') /* 2013년 이전 이월 기부금 */
                                                     AND SETT_FG          = V_SETT_FG
                                                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                     AND CNTRIB_TYPE_CD = 'A032400001'  --법정기부금
             ...
             -- 16~18 이월금액 
                 IF V_CNTRIB_10_PRE_SUM_AMT > 0 THEN     
                    --법정공제대상금액한도 V_LMT_CNTRIB_10_AMT
                    V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT - V_CNTRIB_10_PPRE_SUM_AMT);
                    V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_PRE_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제 대상금액
                      IF V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                              IF V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT > 20000000 THEN --직전 누적이 2천 넘을때
                                    V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT  * 0.30));         -- 2천 초과 30%
                              ELSE                                                        --누적이 2천 안될경우
                                  IF V_LMT_CNTRIB_SUM_AMT > 20000000 THEN --누적이 2천 넘을때
                                        V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 20000000)  * 0.30));         -- 2천 초과 30%
                                        V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (20000000 - (V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT) )  * 0.15));  -- 2천 이하 15%
                                  ELSE                                                        --누적이 2천 안될경우
                                        V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT  * 0.15));                                 -- 2천 이하 15%
                                  END IF;
                              END IF;

                          V_FLAW_CNTRIB_PRE_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;
                      END IF;

                        DBMS_OUTPUT.PUT_LINE('.            @@법정  16,18        입력금액  : ' || V_CNTRIB_10_PRE_SUM_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  16,18        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  16,18   공제대상금액  : ' || V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  16,18         공제금액  : ' || V_FLAW_CNTRIB_PRE_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액                       한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                        2천이하  : ' || V_CNTRIB_DOWN_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                        2천초과  : ' || V_CNTRIB_UP_AMT);

                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PRE_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PRE_TAXDUC_AMT);
                          ELSE
                            V_FLAW_CNTRIB_PRE_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                            IF V_FLAW_CNTRIB_PRE_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                            ELSE
                                V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_FLAW_CNTRIB_PRE_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.30);
                            END IF;
                    ...
                    --################################################################
                        ...
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  16~18 공제대상금액  : ' || V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  16~18      공제금액  : ' || V_FLAW_CNTRIB_PRE_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리                          한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                        ...
                        IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                                ...
                                IF V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                      BEGIN 
                                              V_TMP_CNT := 0;
                                              V_TMP_AMT := V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT; --(35)기부금(이월분)
                                              FOR REC_IWOL IN
                                              (
                                                  SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                           NVL(CNTRIB_PREAMT,0) CNTRIB_PREAMT,   --전년까지 공제금액
                                                   ...
                                                   WHERE YY                 = IN_YY
                                                     --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                     AND CNTRIB_YY       in ('2016','2017','2018') /* 2013년 이전 이월 기부금 */
                                                     AND SETT_FG          = V_SETT_FG
                                                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                     AND CNTRIB_TYPE_CD = 'A032400001'  --법정기부금
             ...
             -- 2019 당해 금액 
                 IF V_CNTRIB_10_YEAR_SUM_AMT > 0 THEN     
                    --법정공제대상금액한도 V_LMT_CNTRIB_10_AMT
                    V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT - V_CNTRIB_10_PRE_SUM_AMT);

DBMS_OUTPUT.PUT_LINE('.            eterain법정  당년        한도금액  : V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT - V_CNTRIB_10_PRE_SUM_AMT)');
DBMS_OUTPUT.PUT_LINE('.            eterain법정  당년        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain법정  당년        한도금액  V_CNTRIB_10_PPRE_SUM_AMT : ' || V_CNTRIB_10_PPRE_SUM_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain법정  당년        한도금액  V_CNTRIB_10_PRE_SUM_AMT  : ' || V_CNTRIB_10_PRE_SUM_AMT);

                    V_FLAW_CNTRIB_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_YEAR_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제 대상금액
                      IF V_FLAW_CNTRIB_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_FLAW_CNTRIB_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                              IF V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_DUC_OBJ_AMT > 10000000 THEN --직전 누적이 1천 넘을때
                                    V_CNTRIB_UP_AMT        := GREATEST(0,  FLOOR( V_FLAW_CNTRIB_DUC_OBJ_AMT * 0.30));         -- 1천 초과 30%
                              ELSE                                                        --누적이 1천 안될경우

                                  IF V_LMT_CNTRIB_SUM_AMT > 10000000 THEN --누적이 1천 넘을때
                                        V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 10000000)  * 0.30));         -- 1천 초과 30%
                                        V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (10000000 - (V_LMT_CNTRIB_SUM_AMT - V_FLAW_CNTRIB_DUC_OBJ_AMT) )  * 0.15));  -- 1천 이하 15%
                                  ELSE                                                        --누적이 1천 안될경우
                                        V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_FLAW_CNTRIB_DUC_OBJ_AMT  * 0.15));                                 -- 1천 이하 15%
                                  END IF;

                              END IF;

                          V_FLAW_CNTRIB_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;
                      END IF;

                        DBMS_OUTPUT.PUT_LINE('.            @@법정  당년        입력금액  : ' || V_CNTRIB_10_YEAR_SUM_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  당년        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  당년    공제대상금액  : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  당년    세액공제금액  : ' || V_FLAW_CNTRIB_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            세액                한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                 1천이하  : ' || V_CNTRIB_DOWN_AMT);
                        DBMS_OUTPUT.PUT_LINE('.                                 1천초과  : ' || V_CNTRIB_UP_AMT);

                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_TAXDUC_AMT);
                          ELSE
                            V_FLAW_CNTRIB_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                            IF V_FLAW_CNTRIB_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                V_FLAW_CNTRIB_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                            ELSE
                                V_FLAW_CNTRIB_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_FLAW_CNTRIB_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.30);
                            END IF;
                    ...
                    --################################################################
                        ...
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  당년 공제대상금액  : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  당년 세액공제금액  : ' || V_FLAW_CNTRIB_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리                 한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                        ...
                        IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                                ...
                                V_TMP_CNT := 0;
                                V_TMP_AMT := V_CNTRIB_10_YEAR_SUM_AMT - V_FLAW_CNTRIB_DUC_OBJ_AMT; --(35)기부금(이월분)
                                IF V_TMP_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                      BEGIN 
...
--eterain6                                                     V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_OVERAMT;
                                                     V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_GIAMT;
                                                     
                                                     IF V_TMP_AMT >= 0 THEN
                                                          UPDATE PAYM436
                                                          
--eterain6                                                             SET CNTRIB_OVERAMT = 0 --당년이월금액 
--eterain6                                                               , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
--eterain6                                                               , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                             SET CNTRIB_OVERAMT = 0
                                                               , CNTRIB_PREAMT  = 0
                                                               , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_GIAMT
                                                           ...
                                                           WHERE YY             = IN_YY
                                                             ...
                                                             ;
                                                     ELSE
                                                            IF V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT >= 0 THEN
                                                                 -- 당년 소멸금액 법이 어캐 바뀔지 모르니 2022년에..바꿔도 될듯   
                                                                  UPDATE PAYM436
                                                                  
--eterain6                                                                     SET CNTRIB_OVERAMT = CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT)  --당년이월금액  CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY <= 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 
--eterain6                                                                       , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
--eterain6                                                                       , CNTRIB_GONGAMT = V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                                     SET CNTRIB_OVERAMT = CNTRIB_GIAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT) 
                                                                       , CNTRIB_PREAMT  = 0
                                                                       , CNTRIB_GONGAMT = V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT 
                        ...
                        ELSE
                                ...
                                IF V_FLAW_CNTRIB_DUC_OBJ_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                      BEGIN 
                                              V_TMP_CNT := 0;                                              
                                              V_TMP_AMT := V_FLAW_CNTRIB_DUC_OBJ_AMT; --(35)기부금(이월분)
...
--eterain6                                                     V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_OVERAMT;
                                                     V_TMP_AMT := V_TMP_AMT - REC_IWOL.CNTRIB_GIAMT;
DBMS_OUTPUT.PUT_LINE('eterain1 ---> V_TMP_AMT : ' || V_TMP_AMT);
DBMS_OUTPUT.PUT_LINE('eterain1 ---> REC_IWOL.CNTRIB_GIAMT : ' || REC_IWOL.CNTRIB_GIAMT);
...
--eterain6                                                               , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                           ...
                                                           WHERE YY             = IN_YY
                                                             ...
                                                             AND CNTRIB_TYPE_CD = REC_IWOL.CNTRIB_TYPE_CD
                                                             ;
DBMS_OUTPUT.PUT_LINE('eterain1 ---> REC_IWOL.CNTRIB_GIAMT : ' || REC_IWOL.CNTRIB_YY || ' , ' || REC_IWOL.CNTRIB_GIAMT);
...
--eterain6                                                                       , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
--eterain6                                                                       , CNTRIB_GONGAMT = V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
--eterain6                                                                       , CNTRIB_DESTAMT = CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY > 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 

                                                                     SET CNTRIB_OVERAMT = REC_IWOL.CNTRIB_GIAMT - V_TMP_AMT  
                                                                       , CNTRIB_PREAMT  = 0
                                                                       , CNTRIB_GONGAMT = V_TMP_AMT
                                                                       , CNTRIB_DESTAMT = 0
                                                            ...
                                                            V_TMP_AMT := 0;
DBMS_OUTPUT.PUT_LINE('eterain1 ---> REC_IWOL.CNTRIB_GIAMT : ' || REC_IWOL.CNTRIB_YY || ' , ' || REC_IWOL.CNTRIB_GIAMT);
DBMS_OUTPUT.PUT_LINE('eterain2 ---> V_TMP_AMT             : ' || V_TMP_AMT);
                                                     END IF;
...
-- richardoh 20200217 04:00  여기까지 작업완료 
-- richardoh 20200217 22:16  법정 고액 세액처리 완료.... 

--richardoh 지정기부금 종교외 (종교단체 기부금이 없는경우)

               IF V_CNTRIB_41_2013_SUM_AMT + V_CNTRIB_41_PPRE_SUM_AMT + V_CNTRIB_41_PRE_SUM_AMT + V_CNTRIB_41_YEAR_SUM_AMT  < 1 THEN
                    DBMS_OUTPUT.PUT_LINE('.         ####지정기부금 (종교단체외 ) 종교단체 기부금이 없는경우 ');
                    --기부금공제대상금액 한도 지정 (근로소득금액  - 정치-법정 )*0.3. 소득공제에서 한도 산정했음.
                    --V_LMT_CNTRIB_4041_AMT := GREATEST(0, V_LMT_CNTRIB_4041_AMT ); 

                    -- 14,15 이월금액 
                     IF V_CNTRIB_40_PPRE_SUM_AMT > 0 THEN    --입력금액 
                        --지정공제대상금액한도 V_LMT_CNTRIB_4041_AMT
                        V_LMT_CNTRIB_4041_AMT     :=GREATEST(0,  V_LMT_CNTRIB_4041_AMT ); --한도금액 
                        V_CNTRIB_40_PPRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_40_PPRE_SUM_AMT    , V_LMT_CNTRIB_4041_AMT       ); --공제대상금액
                          IF V_CNTRIB_40_PPRE_DUC_OBJ_AMT > 0 THEN
                              V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_CNTRIB_40_PPRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)


                                  IF V_LMT_CNTRIB_SUM_AMT - V_CNTRIB_40_PPRE_DUC_OBJ_AMT > 30000000 THEN --직전 누적이 3천 넘을때
                                        V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( V_CNTRIB_40_PPRE_DUC_OBJ_AMT  * 0.25));         -- 3천 초과 25%
                                  ELSE  
                                      IF V_LMT_CNTRIB_SUM_AMT > 30000000 THEN --누적이 3000만원을 초과하면
                                            V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 30000000)  * 0.25));         -- 3천 초과 25%
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (30000000 - (V_LMT_CNTRIB_SUM_AMT - V_CNTRIB_40_PPRE_DUC_OBJ_AMT) )  * 0.15));  -- 3천 이하 15%
                                      ELSE                                                     --누적이 3천 안될경우
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_CNTRIB_40_PPRE_DUC_OBJ_AMT  * 0.15));                                 -- 3천 이하 15%
                                      END IF;
                                  END IF;

                              V_CNTRIB_40_PPRE_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;             --공제금액 
                          END IF;

                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  14,15        입력금액  : ' || V_CNTRIB_40_PPRE_SUM_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  14,15        한도금액  : ' || V_LMT_CNTRIB_4041_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  14,15  공제대상금액  : ' || V_CNTRIB_40_PPRE_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  14,15         공제금액  : ' || V_CNTRIB_40_PPRE_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액                       한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                        3천이하  : ' || V_CNTRIB_DOWN_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                        3천초과  : ' || V_CNTRIB_UP_AMT);

                        --###################### 지정(종교외)기부금 ############################
                              IF (V_CAL_TDUC_TEMP_AMT - V_CNTRIB_40_PPRE_TAXDUC_AMT) >= 0 THEN  --지정(종교외)기부금
                                 V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_CNTRIB_40_PPRE_TAXDUC_AMT);
                              ELSE
                                V_CNTRIB_40_PPRE_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                                IF V_CNTRIB_40_PPRE_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                    V_CNTRIB_40_PPRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                                ELSE
                                    V_CNTRIB_40_PPRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_CNTRIB_40_PPRE_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.25);
                                END IF;
                        ...
                        --################################################################
                            ...
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  14,15 공제대상금액  : ' || V_CNTRIB_40_PPRE_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  14,15      공제금액  : ' || V_CNTRIB_40_PPRE_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리                        한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            ...
                            IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                                    ...
                                    IF V_CNTRIB_40_PPRE_DUC_OBJ_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                          BEGIN 
                                                  V_TMP_CNT := 0;
                                                  V_TMP_AMT := V_CNTRIB_40_PPRE_DUC_OBJ_AMT; --(35)기부금(이월분)
                                                  FOR REC_IWOL IN
                                                  (
                                                      SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                               NVL(CNTRIB_PREAMT,0) CNTRIB_PREAMT,   --전년까지 공제금액
                                                       ...
                                                       WHERE YY                 = IN_YY
                                                         --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                         AND CNTRIB_YY       in ('2014','2015') /* 1415이월 기부금 */
                                                         AND SETT_FG          = V_SETT_FG
                                                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                         AND CNTRIB_TYPE_CD = 'A032400006'  --지정(종교외)기부금
                                                         --AND SETT_FG = 'A031300001'--연말정산인것만
                                                         AND NVL(CNTRIB_OVERAMT,0) <> 0 --전년도 결과 중 이월금액이 0 이 아닌 내역만 당년도 공제 내역에 갖고 옴
                                                         ORDER BY CNTRIB_YY
                                                  )
                                          ...
                                          BEGIN 
                                                  ...
                                                  (
                                                      SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                       ...
                                                       WHERE YY                 = IN_YY
                                                         AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                         AND CNTRIB_YY       IN ('2014','2015') /* 1415 이월 기부금 */
                                                         AND SETT_FG          = V_SETT_FG
                                                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                         AND CNTRIB_TYPE_CD = 'A032400006'  --지정(종교외)기부금
                 ...
                 -- 16~18 이월금액    
                     IF V_CNTRIB_40_PRE_SUM_AMT > 0 THEN    --입력금액 
                        --지정공제대상금액한도 V_LMT_CNTRIB_4041_AMT
                        V_LMT_CNTRIB_4041_AMT     :=GREATEST(0,  V_LMT_CNTRIB_4041_AMT - V_CNTRIB_40_PPRE_DUC_OBJ_AMT ); --한도금액 
                        V_CNTRIB_40_PRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_40_PRE_SUM_AMT    , V_LMT_CNTRIB_4041_AMT       ); --공제대상금액
                          IF V_CNTRIB_40_PRE_DUC_OBJ_AMT > 0 THEN
                              V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_CNTRIB_40_PRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)


                                  IF V_LMT_CNTRIB_SUM_AMT - V_CNTRIB_40_PRE_DUC_OBJ_AMT > 20000000 THEN --직전 누적이 3천 넘을때
                                        V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( V_CNTRIB_40_PRE_DUC_OBJ_AMT  * 0.30));         -- 2천 초과 30%
                                  ELSE  
                                      IF V_LMT_CNTRIB_SUM_AMT > 20000000 THEN --누적이 2000만원을 초과하면
                                            V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 20000000)  * 0.30));         -- 2천 초과 30%
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (20000000 - (V_LMT_CNTRIB_SUM_AMT - V_CNTRIB_40_PRE_DUC_OBJ_AMT) )  * 0.15));  -- 3천 이하 15%
                                      ELSE                                                     --누적이 2천 안될경우
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_CNTRIB_40_PRE_DUC_OBJ_AMT  * 0.15));                                 -- 3천 이하 15%
                                      END IF;
                                  END IF;

                              V_CNTRIB_40_PRE_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;             --공제금액 
                          END IF;

                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  16~18        입력금액  : ' || V_CNTRIB_40_PRE_SUM_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  16~18        한도금액  : ' || V_LMT_CNTRIB_4041_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  16~18  공제대상금액  : ' || V_CNTRIB_40_PRE_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  16~18         공제금액  : ' || V_CNTRIB_40_PRE_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액                                  한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                                  2천이하  : ' || V_CNTRIB_DOWN_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                                  2천초과  : ' || V_CNTRIB_UP_AMT);

                        --###################### 지정(종교외)기부금 16~18############################
                              IF (V_CAL_TDUC_TEMP_AMT - V_CNTRIB_40_PRE_TAXDUC_AMT) >= 0 THEN  --지정(종교외)기부금16~18
                                 V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_CNTRIB_40_PRE_TAXDUC_AMT);
                              ELSE
                                V_CNTRIB_40_PRE_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                                IF V_CNTRIB_40_PRE_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                    V_CNTRIB_40_PRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                                ELSE
                                    V_CNTRIB_40_PRE_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_CNTRIB_40_PRE_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.25);
                                END IF;
                        ...
                        --################################################################
                            ...
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  16~18 공제대상금액  : ' || V_CNTRIB_40_PRE_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  16~18      공제금액  : ' || V_CNTRIB_40_PRE_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리                                   한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            ...
                            IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                                    ...
                                    IF V_CNTRIB_40_PRE_DUC_OBJ_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                          BEGIN 
                                                  V_TMP_CNT := 0;
                                                  V_TMP_AMT := V_CNTRIB_40_PRE_DUC_OBJ_AMT; --(35)기부금(이월분)
                                                  FOR REC_IWOL IN
                                                  (
                                                      SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                               NVL(CNTRIB_PREAMT,0) CNTRIB_PREAMT,   --전년까지 공제금액
                                                       ...
                                                       WHERE YY                 = IN_YY
                                                         --AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                         AND CNTRIB_YY       in ('2016','2017','2018') /* 16~18이월 기부금 */
                                                         AND SETT_FG          = V_SETT_FG
                                                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                         AND CNTRIB_TYPE_CD = 'A032400006'  --지정(종교외)기부금
                                          ...
                                          BEGIN 
                                                  ...
                                                  (
                                                      SELECT NVL(CNTRIB_OVERAMT,0) CNTRIB_OVERAMT,   --당년 이월금액
                                                       ...
                                                       WHERE YY                 = IN_YY
                                                         AND YRETXA_SEQ     = IN_YRETXA_SEQ /*@VER.2017_0*/
                                                         AND CNTRIB_YY       IN ('2016','2017','2018') /* 16~18 이월 기부금 */
                                                         AND SETT_FG          = V_SETT_FG
                                                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                                                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                                                         AND CNTRIB_TYPE_CD = 'A032400006'  --지정(종교외)기부금
                 ...
                 -- 2019 당해 금액 지정(종교외) 당년 
                     IF V_CNTRIB_40_YEAR_SUM_AMT > 0 THEN     
                        --지정(종교외)공제대상금액한도 V_LMT_CNTRIB_4041_AMT
                        V_LMT_CNTRIB_4041_AMT     :=GREATEST(0,  V_LMT_CNTRIB_4041_AMT - V_CNTRIB_40_PPRE_DUC_OBJ_AMT - V_CNTRIB_40_PRE_DUC_OBJ_AMT ); --한도금액
                        V_APNT_CNTRIB40_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_40_YEAR_SUM_AMT    , V_LMT_CNTRIB_4041_AMT       ); --공제 대상금액
                          IF V_APNT_CNTRIB40_DUC_OBJ_AMT > 0 THEN
                              V_LMT_CNTRIB_SUM_AMT      := V_LMT_CNTRIB_SUM_AMT + V_APNT_CNTRIB40_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                                  IF V_LMT_CNTRIB_SUM_AMT - V_APNT_CNTRIB40_DUC_OBJ_AMT > 10000000 THEN --직전 누적이 1천 넘을때
                                        V_CNTRIB_UP_AMT        := GREATEST(0,  FLOOR( V_APNT_CNTRIB40_DUC_OBJ_AMT * 0.30));         -- 1천 초과 30%
                                  ELSE                                                        --누적이 1천 안될경우

                                      IF V_LMT_CNTRIB_SUM_AMT > 10000000 THEN --누적이 1천 넘을때
                                            V_CNTRIB_UP_AMT            := GREATEST(0,  FLOOR( (V_LMT_CNTRIB_SUM_AMT - 10000000)  * 0.30));         -- 1천 초과 30%
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR( (10000000 - (V_LMT_CNTRIB_SUM_AMT - V_APNT_CNTRIB40_DUC_OBJ_AMT) )  * 0.15));  -- 1천 이하 15%
                                      ELSE                                                        --누적이 1천 안될경우
                                            V_CNTRIB_DOWN_AMT      := GREATEST(0,  FLOOR(V_APNT_CNTRIB40_DUC_OBJ_AMT  * 0.15));                                 -- 1천 이하 15%
                                      END IF;

                                  END IF;

                              V_APNT_CNTRIB40_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;
                          END IF;

                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  당년        입력금액  : ' || V_CNTRIB_40_YEAR_SUM_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  당년        한도금액  : ' || V_LMT_CNTRIB_4041_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  당년    공제대상금액  : ' || V_APNT_CNTRIB40_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.            @@지정(종교외)  당년        공제금액  : ' || V_APNT_CNTRIB40_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액                              한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                                1천이하  : ' || V_CNTRIB_DOWN_AMT);
                            DBMS_OUTPUT.PUT_LINE('.                                                1천초과  : ' || V_CNTRIB_UP_AMT);

                        --###################### 지정(종교외)기부금 ############################
                              IF (V_CAL_TDUC_TEMP_AMT - V_APNT_CNTRIB40_TAXDUC_AMT) >= 0 THEN  --지정(종교외)기부금
                                 V_CAL_TDUC_TEMP_AMT :=  (V_CAL_TDUC_TEMP_AMT - V_APNT_CNTRIB40_TAXDUC_AMT);
                              ELSE
                                V_APNT_CNTRIB40_TAXDUC_AMT := V_CAL_TDUC_TEMP_AMT;

                                IF V_APNT_CNTRIB40_TAXDUC_AMT <= V_CNTRIB_DOWN_AMT THEN
                                    V_APNT_CNTRIB40_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15);
                                ELSE
                                    V_APNT_CNTRIB40_DUC_OBJ_AMT := TRUNC(V_CNTRIB_DOWN_AMT / 0.15) + TRUNC(( V_APNT_CNTRIB40_TAXDUC_AMT - V_CNTRIB_DOWN_AMT ) / 0.30);
                                END IF;
                        ...
                        --################################################################
                            ...
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  당년 공제대상금액  : ' || V_APNT_CNTRIB40_DUC_OBJ_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리@@지정(종교외)  당년     공제금액  : ' || V_APNT_CNTRIB40_TAXDUC_AMT);
                            DBMS_OUTPUT.PUT_LINE('.           세액처리                      한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                            ...
                            IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                                  ...
                                  V_TMP_CNT := 0;
                                  V_TMP_AMT := V_CNTRIB_40_YEAR_SUM_AMT - V_APNT_CNTRIB40_DUC_OBJ_AMT; --(35)기부금(이월분)
                                    IF V_TMP_AMT >= 0 THEN -- 공제금액이 있거나 0일경우     
                                          BEGIN 
    ...
    --eterain6                                                                       , CNTRIB_PREAMT     = REC_IWOL.CNTRIB_GIAMT - REC_IWOL.CNTRIB_OVERAMT -- 전년까지 공제금액 ( 최초 기부금액에서  전년도 이월액을 뺀다)
    --eterain6                                                                       , CNTRIB_GONGAMT = V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                                         SET CNTRIB_OVERAMT = V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT  --당년이월금액  CASE WHEN  IN_YY - REC_IWOL.CNTRIB_YY <= 10  THEN CNTRIB_OVERAMT - (V_TMP_AMT + REC_IWOL.CNTRIB_OVERAMT) ELSE 0 END 
                                                                           , CNTRIB_PREAMT  = 0
                                                                           , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_GIAMT - ( V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT ) -- 당년공제금액
                            ...
                            ELSE
                                  ...
                                  V_TMP_CNT := 0;
                                  V_TMP_AMT := V_CNTRIB_40_YEAR_SUM_AMT - V_APNT_CNTRIB40_DUC_OBJ_AMT; --(35)기부금(이월분)
                                  DBMS_OUTPUT.PUT_LINE('.       @지정(종교외) 당년    이월금액  : ' || V_TMP_AMT);
    ...
    --eterain6                                                               , CNTRIB_GONGAMT = REC_IWOL.CNTRIB_OVERAMT -- 당년공제금액
                                                         ...
                                                         ELSE
                                                                IF V_TMP_AMT + REC_IWOL.CNTRIB_GIAMT >= 0 THEN
                                                                     ...
                                                                     DBMS_OUTPUT.PUT_LINE('.       @지정(종교외) 당년    V_TMP_AMT  REC_IWOL.CNTRIB_GIAMT  : ' || V_TMP_AMT || ':'|| REC_IWOL.CNTRIB_GIAMT);
                                                                     DBMS_OUTPUT.PUT_LINE('.       @지정(종교외) 당년    V_TMP_AMT  : ' || V_TMP_AMT);
                                                                     DBMS_OUTPUT.PUT_LINE('.       @지정(종교외) 당년    REC_IWOL.CNTRIB_GIAMT  : ' || REC_IWOL.CNTRIB_GIAMT);
                                                                     DBMS_OUTPUT.PUT_LINE('.       @지정(종교외) 당년    이월금액  : ' || V_TMP_AMT);
               ...
               ELSE
                  DBMS_OUTPUT.PUT_LINE('.         ####지정기부금 (종교단체 ) 종교단체 기부금이 있는경우 ');
                    V_LMT_CNTRIB_4041_AMT := GREATEST(0, V_LMT_CNTRIB_4041_AMT ); --기부금공제대상금액 한도 지정 . 소득공제에서 한도 산정했음.



               END IF;

--richardoh 20200218 1:30 여기까지 작업
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
            ...
            END IF;
/* ZZODEM */
/*V_OCCR_LOC_NM   := '법정기부금 체크:5';
V_DB_ERROR_CTNT := 'V_FLAW_CNTRIB_TAXDUC_AMT:'||V_FLAW_CNTRIB_TAXDUC_AMT||chr(13)||chr(10)||
                   'V_FLAW_CNTRIB_DUC_OBJ_AMT:'||V_FLAW_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_TAXDUC_AMT:'||V_APNT_CNTRIB_TAXDUC_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */



            V_STAD_TAXDUC_OBJ_AMT := V_STAD_TAXDUC_OBJ_AMT + V_FLAW_CNTRIB_DUC_OBJ_AMT + V_APNT_CNTRIB_DUC_OBJ_AMT;  /*****/
...
--기부금 이월 update 여기요--------------------------------------------------------------------------
            IF ( IN_SETT_FG = 'A031300003' ) THEN  --연말정산 시뮬레이션인 경우
                ...
                /**[시뮬:소득공제] (35)기부금(이월분):V_CNTRIB_AMT_CYOV_AMT 2013년이전 이월기부금)update***법정기부금,종교단체외 지정기부금,종교단체 지정기부금 순.************/
                IF V_CNTRIB_AMT_CYOV_AMT = 0 THEN
                   V_TMP_STEP := 'U2013_0';
                   /* 2013년 이전 기부금(이월분)이 0 일경우 2013년이전 이월분 모두 미공제 처리*/
                   UPDATE PAYM436
                     SET CNTRIB_GONGAMT = 0
                     /*  (기존)
                       , CNTRIB_OVERAMT = CASE WHEN (CNTRIB_TYPE_CD  = 'A032400001' AND CNTRIB_YY = IN_YY - 3) OR
                                                    (CNTRIB_TYPE_CD <> 'A032400001' AND CNTRIB_YY = IN_YY - 5)
                         @VER.2019_5 기부금 이월은 5년 -> 10년으로 변경
                                                    */
                       , CNTRIB_OVERAMT = CASE WHEN (CNTRIB_TYPE_CD  = 'A032400001' AND CNTRIB_YY = IN_YY - 4) OR
                                                    (CNTRIB_TYPE_CD <> 'A032400001' AND CNTRIB_YY = IN_YY - 10)
                                               THEN CNTRIB_GIAMT - CNTRIB_PREAMT    -- (@VER.2018_11_1)적용
                                               ELSE 0                               -- (@VER.2018_11_1)적용
                                          END
                     /* (기존)
                       , CNTRIB_DESTAMT = CASE WHEN (CNTRIB_TYPE_CD  = 'A032400001' AND CNTRIB_YY = IN_YY - 3) OR
                                                    (CNTRIB_TYPE_CD <> 'A032400001' AND CNTRIB_YY = IN_YY - 5)
                         @VER.2019_5 기부금 이월은 5년 -> 10년으로 변경
                                                    */
                       , CNTRIB_DESTAMT = CASE WHEN (CNTRIB_TYPE_CD  = 'A032400001' AND CNTRIB_YY = IN_YY - 4) OR
                                                    (CNTRIB_TYPE_CD <> 'A032400001' AND CNTRIB_YY = IN_YY - 10)
                                               THEN 0                               -- (@VER.2018_11_1)적용
                                               ELSE CNTRIB_GIAMT - CNTRIB_PREAMT    -- (@VER.2018_11_1)적용
                                          END
                   WHERE YY             = IN_YY
                     AND CNTRIB_YY     <= '2013' /* 2013년 이전 기부금(소득공제 대상)*/
                     AND SETT_FG        = V_SETT_FG
                     AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                     AND RPST_PERS_NO   = REC.RPST_PERS_NO;
                ELSE
                  /*2013년이전 이월기부금 공제금 계산*/
                  BEGIN
                      V_TMP_CNT := 0;
                      V_TMP_AMT := V_CNTRIB_AMT_CYOV_AMT; --(35)기부금(이월분)
                      FOR REC_IWOL_2013 IN
                      (
                          SELECT CNTRIB_YY
                                ,CNTRIB_TYPE_CD
                            FROM PAYM436
                           WHERE YY             = IN_YY
                             ...
                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                             AND RPST_PERS_NO   = REC.RPST_PERS_NO
                        ORDER BY CNTRIB_TYPE_CD, CNTRIB_YY /*기부금코드(법정10,지정(종교외):40, 지정(종교))순, 년도 순*/
                      )
                      LOOP
                        ...
                        IF V_TMP_CNT > 1 THEN
                           IF V_TMP_AMT_2013 <= 0 THEN
                              V_TMP_AMT := 0;
                           ELSE
                              V_TMP_AMT := GREATEST(V_TMP_AMT - V_TMP_AMT_2013, 0);
                           END IF;
                        END IF;

                       /* ①:2013년이전 이월기부금 소득공제액 - 2013년이전 이월기부금(기부금코드, 년도순)*/
                        SELECT GREATEST(V_TMP_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                          INTO V_TMP_AMT_2013
                          FROM PAYM436
                         WHERE YY             = IN_YY
                           AND CNTRIB_YY      = REC_IWOL_2013.CNTRIB_YY
                           AND SETT_FG        = V_SETT_FG
                           AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                           AND RPST_PERS_NO   = REC.RPST_PERS_NO
                           AND CNTRIB_TYPE_CD = REC_IWOL_2013.CNTRIB_TYPE_CD
                           ;

                          /*  ①금액이 0보다 클 경우 기부액 전액 공제 : 2013년이전 이월기부금(기부금코드,년도순) */
                          IF V_TMP_AMT_2013 > 0 THEN
                              V_TMP_STEP := 'U2013_1';
                              UPDATE PAYM436
                                 SET CNTRIB_GONGAMT = CNTRIB_GIAMT - CNTRIB_PREAMT
                                   , CNTRIB_OVERAMT = 0
                                   , CNTRIB_DESTAMT = 0
                               WHERE YY             = IN_YY
                                 AND CNTRIB_YY      = REC_IWOL_2013.CNTRIB_YY
                              ...
                              /*다음 자료를 위해 변수 설정.*/
                              SELECT CNTRIB_GIAMT - CNTRIB_PREAMT
                                INTO V_TMP_AMT_2013
                                FROM PAYM436
                               WHERE YY             = IN_YY
                                 AND CNTRIB_YY      = REC_IWOL_2013.CNTRIB_YY
                                 ...
                                 ;
                          ELSE
                             /* ①금액이 0이하일 경우 ①금액(V_TMP_AMT) 공제 : 2013년이전 이월기부금(기부금코드,년도순) */
                              V_TMP_STEP := 'U2013_2';
                              UPDATE PAYM436
                                 SET CNTRIB_GONGAMT = V_TMP_AMT
                                 /*  (기존)
                                   , CNTRIB_OVERAMT = CASE WHEN (REC_IWOL_2013.CNTRIB_TYPE_CD  = 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 3) OR
                                                                (REC_IWOL_2013.CNTRIB_TYPE_CD <> 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 5)
                                    @VER.2019_5 기부금 이월은 5년 -> 10년으로 변경                          */
                                   , CNTRIB_OVERAMT = CASE WHEN (REC_IWOL_2013.CNTRIB_TYPE_CD  = 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 4) OR
                                                                (REC_IWOL_2013.CNTRIB_TYPE_CD <> 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 10)
                                                           THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP_AMT    -- (@VER.2018_11_1)적용
                                                           ELSE 0                                             -- (@VER.2018_11_1)적용
                                                      END
                                /*   (기존)
                                , CNTRIB_DESTAMT = CASE WHEN (REC_IWOL_2013.CNTRIB_TYPE_CD  = 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 3) OR
                                                                (REC_IWOL_2013.CNTRIB_TYPE_CD <> 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 5)
                                    @VER.2019_5 기부금 이월은 5년 -> 10년으로 변경                          */
                                   , CNTRIB_DESTAMT = CASE WHEN (REC_IWOL_2013.CNTRIB_TYPE_CD  = 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 4) OR
                                                                (REC_IWOL_2013.CNTRIB_TYPE_CD <> 'A032400001' AND REC_IWOL_2013.CNTRIB_YY = IN_YY - 10)
                                                           THEN 0                                             -- (@VER.2018_11_1)적용
                                                           ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP_AMT    -- (@VER.2018_11_1)적용
                                                      END     /*소멸조건:2013년이전기부금 법정기부금(3년)/지정기부금(5년)*/
                               WHERE YY             = IN_YY
                                 AND CNTRIB_YY      = REC_IWOL_2013.CNTRIB_YY
                                 AND SETT_FG        = V_SETT_FG
                                 AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
...
--V_OCCR_LOC_NM   := '2013년 이전 이월기부금 체크';
--V_DB_ERROR_CTNT := 'V_CNTRIB_AMT_CYOV_AMT:'||V_CNTRIB_AMT_CYOV_AMT||chr(13)||chr(10)||
--                   'V_TMP_STEP:'||V_TMP_STEP||chr(13)||chr(10)||
--                   'V_TMP_AMT:'||V_TMP_AMT||chr(13)||chr(10)||
--                   'V_TMP_AMT_2013:'||V_TMP_AMT_2013||chr(13)||chr(10);
--SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP );
                  ...
                  /*2014년이후 이월 법정기부금 공제금 계산*/
                  BEGIN
                      V_TMP_CNT := 0;
                       /* 기부금 공제 순서 변경. 이월액 먼저 소멸되도록 변경됨 (@VER.2019_7)
                          법정기부금 대상금액 -  2014년이후 이월법정기부금(년도순) 으로 변경 */
                       IF V_TMP_CNT = 0 THEN
                          V_TMP10_AMT_2014 := V_FLAW_CNTRIB_DUC_OBJ_AMT;

                         --eterain 법정
                         V_TMP_AMT := V_FLAW_CNTRIB_DUC_OBJ_AMT;
                       END IF;

                      FOR REC_10_IWOL_2014 IN
                      (
                          SELECT CNTRIB_YY
                            FROM PAYM436
                           WHERE YY             = IN_YY
                             AND CNTRIB_YY     <> IN_YY  /* 당해 기부금 제외*/
                             AND CNTRIB_YY     >= '2014' /* 2014년 이후 이월 기부금 */
                             AND SETT_FG        = V_SETT_FG
                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                             AND RPST_PERS_NO   = REC.RPST_PERS_NO
                      ...
                      LOOP
                        ...
                        V_TMP_CNT := V_TMP_CNT + 1;
                       /* ②:법정기부금 대상금액 - 당해 법정기부금 - 2014년이후 이월법정기부금(년도순)*/
--                        SELECT GREATEST(V_TMP10_AMT_2014 - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                          SELECT V_TMP_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT)
                          INTO V_TMP10_AMT_2014
                          FROM PAYM436
                         WHERE YY             = IN_YY
                           AND CNTRIB_YY      = REC_10_IWOL_2014.CNTRIB_YY
                           AND SETT_FG        = V_SETT_FG
                           AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                           AND RPST_PERS_NO   = REC.RPST_PERS_NO
                           AND CNTRIB_TYPE_CD = 'A032400001' /*법정(코드:10)*/
                          ...
                          V_TMP_AMT_ETE := V_TMP10_AMT_2014;
                          
                          /*  ②금액이 0보다 클 경우 기부액 전액 공제 : 2014년이후 이월 법정기부금(년도순) */
                          IF V_TMP10_AMT_2014 > 0 THEN
                              V_TMP_STEP := 'U10_2014_1';
                              UPDATE PAYM436
                                 SET CNTRIB_GONGAMT = (CNTRIB_GIAMT - CNTRIB_PREAMT)
                                   , CNTRIB_OVERAMT = 0
                                   , CNTRIB_DESTAMT = 0
                              ...
                              /* 다음자료를 위해 변수 설정*/
                              /* 위에서 계산하므로 삭제 @VER.2019_7
                              SELECT V_TMP10_AMT_2014 - CNTRIB_GIAMT - CNTRIB_PREAMT
                                INTO V_TMP10_AMT_2014
                                FROM PAYM436
                               WHERE YY             = IN_YY
                                 AND CNTRIB_YY      = REC_10_IWOL_2014.CNTRIB_YY
                                ...
                                --eterain 법정
                                --V_TMP_AMT := V_TMP_AMT - V_TMP10_AMT_2014;
                                --V_TMP10_AMT_2014 := V_TMP_AMT;

                          ELSE
                             --eterain 법정
                                SELECT CASE WHEN V_TMP_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT) <= 0 THEN V_TMP_AMT
                                            ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP_AMT
                                       END
                                  INTO V_TMP10_AMT_2014
                                  FROM PAYM436
                                 WHERE YY             = IN_YY
                             ...
                             /* ②금액이 0이하일 경우 ①금액(V_TMP10_AMT) 공제 : 2014년이후 이월 법정기부금(년도순) */
                              V_TMP_STEP := 'UU40_2014_2';
                              UPDATE PAYM436


                                --SET CNTRIB_GONGAMT = V_TMP10_AMT_2014
                                 SET CNTRIB_GONGAMT = (CNTRIB_GIAMT - CNTRIB_PREAMT)   -- V_TMP_AMT
                                   , CNTRIB_OVERAMT = CASE WHEN REC_10_IWOL_2014.CNTRIB_YY = IN_YY - 6 AND V_TMP_AMT > 0
                                                           THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP_AMT
                                                           ELSE 0
                                                      END
                                   , CNTRIB_DESTAMT = CASE WHEN REC_10_IWOL_2014.CNTRIB_YY = IN_YY - 6
                                                           THEN (CNTRIB_GIAMT - CNTRIB_PREAMT)
                                                           ELSE 0
                                                      END     /*소멸조건:2014년이후 기부금(5년)*/
                                                              /* @VER.2019_5 이월공제기간 10년으로 연장(IN_YY - 5년에서 IN_YY - 10로 수정) */
                               WHERE YY             = IN_YY
                                 AND CNTRIB_YY      = REC_10_IWOL_2014.CNTRIB_YY
                                 AND SETT_FG        = V_SETT_FG
                                 AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                    ...
                    /**[시뮬:세액공제] 법정기부(법정:10) (당해,2014년이후 이월) update*********************************************************************************************/
                    /* 기부금 공제 순서 변경. 2014년 이후 이월금액이 0이하일 경우 모두 미공제 처리 @2019_7*/
                    --eterain 법정 IF V_TMP10_AMT_2014 <= 0 THEN
                    IF V_TMP10_AMT_2014 < 0 THEN
                       V_TMP_STEP := 'U10_0';
                       /* 법정기부금 대상금액(세액공제대상)이 0이하 일경우 기부년도에 상관없이 모두 미공제 처리 ( 단,2013년 이전기부금은 제외:특별소득공제)*/
                       /* 기부금 공제 순서 변경. 2014년 이후 이월금액이 0이하일 경우 모두 미공제 처리 @2019_7*/
                       /* 기부금 시뮬과 계산같 계산방식이 상이하여 수정 (@VER.2019_12) */
                       /*
                       UPDATE PAYM436
                         SET CNTRIB_GONGAMT = 0
                           , CNTRIB_OVERAMT  = CASE WHEN CNTRIB_YY = IN_YY - 10
                                                 THEN 0
                                                 ELSE CNTRIB_GIAMT - CNTRIB_PREAMT
                                              END
                           , CNTRIB_DESTAMT = CASE WHEN CNTRIB_YY = IN_YY - 10
                                                 THEN CNTRIB_GIAMT - CNTRIB_PREAMT
                                                 ELSE 0
                                              END
                                              --VER.2019_5 이월공제기간 10년으로 연장(IN_YY - 5에서 IN_YY -10으로 수정)
                       WHERE YY             = IN_YY
                         AND CNTRIB_YY     >= '2014' -- 2014년 이후 기부금(세액공제 대상)
                         AND SETT_FG        = V_SETT_FG
                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                    ...
                    --eterain 법정
                    /*
                    ELSIF V_TMP10_AMT_2014 = 0 THEN
                          UPDATE PAYM436
                             SET CNTRIB_GONGAMT = 0
                               , CNTRIB_OVERAMT = CNTRIB_GIAMT
                           WHERE YY             = IN_YY
                             AND CNTRIB_YY      = IN_YY --당해 기부금 
                             AND SETT_FG        = V_SETT_FG
                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                             AND RPST_PERS_NO   = REC.RPST_PERS_NO
                             AND CNTRIB_TYPE_CD = 'A032400001' --법정(코드:10)
                    ...
                    ELSE
                      IF  V_TMP_AMT > 0 THEN
                          V_TMP_AMT := V_TMP_AMT_ETE;
                          V_TMP10_AMT_2014 := 0; 
                      END IF;
                  
                      BEGIN
                        /* ①:법정기부금 대상금액 - 당해 법정기부금 */
                        /* 기부금 공제 순서 변경. 2014년 이후 이월금액으로 처리 @2019_7*/
                        SELECT GREATEST(V_TMP10_AMT_2014 - CNTRIB_GIAMT, 0)
                          INTO V_TMP10_AMT
                          FROM PAYM436
                         WHERE YY             = IN_YY
                           AND CNTRIB_YY      = IN_YY /*당해 기부금 */
                           ...
                           ;
                       EXCEPTION
                            WHEN NO_DATA_FOUND THEN V_TMP10_AMT := V_TMP10_AMT_2014;
                            /* 기부금 공제 순서 변경. 2014년 이후 이월금액으로 처리 @2019_7*/
                       END;

                       /* ①금액이 0보다 클 경우 기부액 전액 공제 : 당해 법정기부금 */
                       IF V_TMP10_AMT > 0 THEN
                          V_TMP_STEP := 'U10_1';
                          UPDATE PAYM436
                             SET CNTRIB_GONGAMT = CNTRIB_GIAMT
                               , CNTRIB_OVERAMT = 0
                               , CNTRIB_DESTAMT = 0
                           WHERE YY             = IN_YY
                             AND CNTRIB_YY      = IN_YY /*당해 기부금 */
                             ...
                             ;
                       ELSE
                        /* ①금액이 0이하일 경우 법정기부공제대상금액 공제 :당해 법정기부금 */
                        /* 기부금 공제 순서 변경. 2014년 이후 이월금액으로 처리 @2019_7*/
                          V_TMP_STEP := 'U10_2';
                          UPDATE PAYM436
                             SET CNTRIB_GONGAMT = V_TMP_AMT
                               , CNTRIB_OVERAMT = CNTRIB_GIAMT - V_TMP_AMT
                           WHERE YY             = IN_YY
                             AND CNTRIB_YY      = IN_YY /*당해 기부금 */
                             AND SETT_FG        = V_SETT_FG
                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                ...
                /**[시뮬:세액공제]지정기부(종교외:40,종교:41) (당해,2014년이후) update*********************************************************************************************/

                /**[계산:세액공제]지정기부(종교외:40,종교:41) (당해,2014년이후) update*********************************************************************************************/
                /* @VER.2017_13 공제순서 변경 (당해 종교외=>이월(2014년~) 종교외=>당해 종교단체=>이월(2014년~) 종교단체*/
                /* 기부금 공제 순서 변경. 이월액 먼저 소멸되도록 변경됨 (@VER.2019_7)
                       (기존) 2013 이월 종교 외 지정 -> 2013 이월 종교 지정 -> 당해 종교 외 지정 -> 2014 이후 이월 종교 외 지정 -> 당해 종교 지정 -> 2014 이후 이월 종교 지정
                       (변경) 2013 이월 종교 외 지정 -> 2013 이월 종교 지정 -> 2014 이후 이월 종교 외 지정 -> 당해 종교 외 지정 -> 2014 이후 이월 종교 지정 -> 당해 종교 지정 */
            IF V_APNT_CNTRIB_DUC_OBJ_AMT = 0 THEN

                   V_TMP_STEP := 'UU4041_0';

                   /*지정기부금 대상금액(세액공제대상)이 0 일경우 기부년도에 상관없이 모두 미공제 처리 ( 단,2013년 이전기부금은 제외:특별소득공제)*/
                   UPDATE PAYM436
                     SET CNTRIB_GONGAMT = 0
                       , CNTRIB_OVERAMT = CASE WHEN CNTRIB_YY = IN_YY - 10
                                               THEN 0
                   ...
                   WHERE YY             = IN_YY
                     ...
                     AND RPST_PERS_NO   = REC.RPST_PERS_NO
                     AND CNTRIB_YY     >= '2014' /* 2014년 이후 기부금(세액공제 대상)*/
                     AND CNTRIB_TYPE_CD IN ('A032400006','A032400007') /*지정기부금:40(종교외),41(종교)*/
                     ;
            ELSE



                  /*2014년이후 이월 지정기부금(종교외) 공제금 계산*/
                  BEGIN
                      V_TMP_CNT := 0;
                      V_TMP40_AMT := V_APNT_CNTRIB_DUC_OBJ_AMT;  --기부금 공제 순서 변경에 따른 초기값 셋팅 @VER.2019_7
                      ...
                      SELECT COUNT(1)
                       ...
                       WHERE YY             = IN_YY
                         ...
                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                         AND CNTRIB_TYPE_CD = 'A032400006' /*지정(코드:40)=종교외*/
                       ;

/*V_OCCR_LOC_NM   := '지정기부 V_APNT_CNTRIB_DUC_OBJ_AMT <> 0';
V_DB_ERROR_CTNT := 'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_TMP40_AMT:'||V_TMP40_AMT||chr(13)||chr(10)||
                   'V_REC_CNT:'||V_REC_CNT||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */

                      FOR REC_40_IWOL_2014 IN
                      (
                          SELECT CNTRIB_YY
                            FROM PAYM436
                           WHERE YY             = IN_YY
                      ...
                      LOOP
                        ...
                        IF V_TMP_CNT > 1 THEN
                           IF V_TMP40_AMT_2014 <= 0 THEN
                              V_TMP40_AMT := 0;
                           ELSE
                              V_TMP40_AMT := GREATEST(V_TMP40_AMT - V_TMP40_AMT_2014, 0);
                           END IF;
                        END IF;
...
V_DB_ERROR_CTNT := 'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_TMP40_AMT:'||V_TMP40_AMT||chr(13)||chr(10)||
                   'V_TMP40_AMT_2014:'||V_TMP40_AMT_2014||chr(13)||chr(10)||
                   'REC_40_IWOL_2014.CNTRIB_YY:'||REC_40_IWOL_2014.CNTRIB_YY||chr(13)||chr(10)
                   ;
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */

                       /* ②:지정기부공제대상금액 - 당해 지정기부금(종교외) - 2014년이후 이월 지정기부금(종교외).. */
                        BEGIN
                            SELECT GREATEST(V_TMP40_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                              INTO V_TMP40_AMT_2014
                              FROM PAYM436
                             WHERE YY             = IN_YY
                               AND CNTRIB_YY      = REC_40_IWOL_2014.CNTRIB_YY
                               AND SETT_FG        = V_SETT_FG
                               AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                               AND RPST_PERS_NO   = REC.RPST_PERS_NO
                               AND CNTRIB_TYPE_CD = 'A032400006' /*지정(코드:40)=종교외*/
                        ...
                        EXCEPTION
                             WHEN NO_DATA_FOUND THEN
                                  IF V_TMP40_AMT > 0 THEN
                                     V_TMP40_AMT_2014 := V_TMP40_AMT;
                                  ELSE
                                     V_TMP40_AMT_2014 := V_APNT_CNTRIB_DUC_OBJ_AMT;
                                  END IF;
                        END;
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                          ...
                          /*  ②금액이 0보다 클 경우 기부액 전액 공제 : 2014년이후 이월 지정기부금(종교외) */
                          /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시 복원처리 */
                          IF V_TMP40_AMT_2014 > 0 THEN
                              V_TMP_STEP := 'U40_2014_1';

/*V_OCCR_LOC_NM   := '지정기부 '|| V_TMP_STEP;
V_DB_ERROR_CTNT := 'V_TMP40_AMT_2014:'||V_TMP40_AMT_2014||chr(13)||chr(10)||
                   'V_TMP40_AMT:'||V_TMP40_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB40_DUC_OBJ_AMT:'||V_APNT_CNTRIB40_DUC_OBJ_AMT||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                              ...
                              /* @VER.2016_지정기부  보완 다음 자료를 위해 변수 설정 (마지막 건수일 경우 설정하지 않는다.) */
                              IF V_TMP_CNT < V_REC_CNT  THEN
                                  SELECT CNTRIB_GIAMT - CNTRIB_PREAMT
                                    INTO V_TMP40_AMT_2014
                                    FROM PAYM436
                                   WHERE YY             = IN_YY
                                     ...
                                     AND CNTRIB_TYPE_CD = 'A032400006' /*지정(코드:40):종교외*/
                                     ;
                              ELSIF V_TMP_CNT = V_REC_CNT THEN
                                /* 마지막 자료일 경우 당해 지정기부금(종교)용 기준금액으로 설정.*/
                                   SELECT SUM(CNTRIB_GONGAMT)
                                     INTO V_TMP40_AMT_2014
                                     FROM PAYM436
                                    WHERE YY             = IN_YY
                                      AND CNTRIB_YY      = REC_40_IWOL_2014.CNTRIB_YY
                                      ...
                                      AND CNTRIB_TYPE_CD = 'A032400006' /*지정(코드:40):종교외*/
                                      ;
                                    V_TMP40_AMT_2014 := GREATEST(V_TMP40_AMT - V_TMP40_AMT_2014);

                              END IF;
                          ELSE
                             /* ②금액이 0이하일 경우 (V_TMP40_AMT) 공제 : 2014년이후 이월 지정기부금(종교외) */
                              V_TMP_STEP := 'U40_2014_2';
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                              UPDATE PAYM436
                                 SET CNTRIB_GONGAMT = V_TMP40_AMT
                                   , CNTRIB_OVERAMT = CASE WHEN REC_40_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                                           THEN 0
                                                           ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT
                                                      END
                                   , CNTRIB_DESTAMT = CASE WHEN REC_40_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                                           THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP40_AMT
                                                           ELSE 0
                                                      END     /*소멸조건:2014년이후 기부금(5년)*/
                                          /* @VER.2019_5 이월공제기간 10년으로 연장(IN_YY - 5년에서 IN_YY - 10 으로 수정) */
                               WHERE YY             = IN_YY
...
/*V_OCCR_LOC_NM   := '지정기부금(종교외) 체크 : 시뮬 ';
V_DB_ERROR_CTNT := 'V_TMP40_AMT_2014:'||V_TMP40_AMT_2014||chr(13)||chr(10)||
                   'V_TMP40_AMT:'||V_TMP40_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT;
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
---------------------------
                  BEGIN
                      /* ①:지정기부공제대상금액 - 당해 지정기부금(종교외) */
                      SELECT GREATEST(V_TMP40_AMT_2014 - CNTRIB_GIAMT, 0)
                        INTO V_TMP40_AMT
                        FROM PAYM436
                       WHERE YY             = IN_YY
                         AND CNTRIB_YY      = IN_YY /*당해 기부금*/
                  ...
                  EXCEPTION
                       WHEN NO_DATA_FOUND THEN V_TMP40_AMT := V_TMP40_AMT_2014;
                  END;

                  /* ①금액이 0보다 크거나 같은 경우 기부액 전액 공제 : 당해 지정기부금(종교외) */
                  /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시복원 */
                  IF V_TMP40_AMT > 0 THEN
                      V_TMP_STEP := 'UU40_1';
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                      ...
                      UPDATE PAYM436
                       ...
                       WHERE YY             = IN_YY
                         ...
                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                         AND CNTRIB_TYPE_CD = 'A032400006'  -- 지정기부금(종교외)
                         ;
                  --eterain
                  ELSE

                      UPDATE PAYM436
                         SET CNTRIB_GONGAMT = CASE WHEN V_TMP40_AMT_2014 < CNTRIB_GIAMT THEN V_TMP40_AMT_2014
                                                   ELSE V_TMP40_AMT_2014 END
                           , CNTRIB_OVERAMT = CASE WHEN V_TMP40_AMT_2014 < CNTRIB_GIAMT THEN CNTRIB_GIAMT - V_TMP40_AMT_2014
                                                   ELSE 0 END
                           , CNTRIB_DESTAMT = 0
                       WHERE YY             = IN_YY
                         AND CNTRIB_YY      = IN_YY /*당해 기부금*/
                  ...
                  /* 당해 지정, 종교기부금 계산시 잔여기부금이 0보다 작은 경우 UPDATE 하지 않는다. 기존 로직 삭제 (@VER.2019_13)
                  --ELSE
                   ①금액이 0미만일 경우 지정기부공제대상금액 공제

                      V_TMP_STEP := 'UU40_2';
                      UPDATE PAYM436

                         SET CNTRIB_GONGAMT = V_APNT_CNTRIB_DUC_OBJ_AMT
                           , CNTRIB_OVERAMT = CNTRIB_GIAMT - V_APNT_CNTRIB_DUC_OBJ_AMT
...
-----------------------------
                      ...
                      /*2014년이후 이월 지정기부금(종교) 공제금 계산*/
                      BEGIN
                        V_TMP_CNT := 0;
                        V_TMP41_AMT := V_TMP40_AMT;   --기부금 공제 순서 변경에 따른 초기값 셋팅 @VER.2019_7

                       /* IF V_REC_CNT = 0 THEN
                            V_TMP41_AMT := V_TMP40_AMT_2014; \* 종교외(2014년이후 이월)가 없을경우 대비 설정*\
                        END IF;*/

                        FOR REC_41_IWOL_2014 IN
                        (
                            SELECT CNTRIB_YY
                              FROM PAYM436
                             WHERE YY             = IN_YY
                               ...
                               AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                               AND RPST_PERS_NO   = REC.RPST_PERS_NO
                               AND CNTRIB_TYPE_CD = 'A032400007' /*지정(코드:41)=종교외*/
                          ORDER BY CNTRIB_YY
                        )
                        LOOP
                          ...
                          IF V_TMP_CNT > 1 THEN
                             IF V_TMP41_AMT_2014 <= 0 THEN
                                V_TMP41_AMT := 0;
                             ELSE
                                V_TMP41_AMT := GREATEST(V_TMP41_AMT - V_TMP41_AMT_2014, 0);
                             END IF;
                          END IF;

                         /* ④:지정기부공제대상금액 -  지정기부금(종교외:당해+이월) -  2014년이후 이월 지정기부금(종교)*/
                          SELECT GREATEST(V_TMP41_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                            INTO V_TMP41_AMT_2014
                            FROM PAYM436
                           WHERE YY             = IN_YY
                             AND CNTRIB_YY      = REC_41_IWOL_2014.CNTRIB_YY
                             AND SETT_FG        = V_SETT_FG
                             AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                             AND RPST_PERS_NO   = REC.RPST_PERS_NO
                             AND CNTRIB_TYPE_CD = 'A032400007' /*지정(코드:41)=종교*/
                             ;



                            /*  ④금액이 0보다 크거나 같을 경우 기부액 전액 공제 : 2014년이후 이월 지정기부금(종교) */
                            /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시복원 */
                            IF V_TMP41_AMT_2014 > 0 THEN
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                                ...
                                SELECT CNTRIB_GIAMT - CNTRIB_PREAMT
                                 ...
                                 WHERE YY             = IN_YY
                                   ...
                                   ;
                            ELSE
                               /* ④금액이 0미만일 경우 V_TMP41_AMT 공제 : 2014년이후 이월 지정기부금(종교) */
                                V_TMP_STEP := 'U41_2014_2';
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                                ...
                                UPDATE PAYM436
                                   SET CNTRIB_GONGAMT = V_TMP41_AMT
                                     , CNTRIB_OVERAMT = CASE WHEN REC_41_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                                             THEN 0
                                                             ELSE (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP41_AMT
                                                        END
                                     , CNTRIB_DESTAMT = CASE WHEN REC_41_IWOL_2014.CNTRIB_YY = IN_YY - 10
                                                             THEN (CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP41_AMT
                                                             ELSE 0
                                                        END     /*소멸조건:2014년이후 기부금(5년)*/
                                          /* @VER.2019_5 이월공제기간 10년으로 연장(IN_YY - 5년에서 IN_YY - 10 으로 수정) */
                                 WHERE YY             = IN_YY
...
---------------------
                  BEGIN
                      /* ③:지정기부공제대상금액 - 지정기부금(종교외:당해+이월) - 당해 지정기부금(종교) */
                      SELECT GREATEST(V_TMP41_AMT_2014 - CNTRIB_GIAMT, 0)
                        INTO V_TMP41_AMT
                        FROM PAYM436
                       WHERE YY             = IN_YY
                         AND CNTRIB_YY      = IN_YY /*당해 기부금*/
                         ...
                         ;
                  EXCEPTION
                       /* 당해 지정(종교)가 없어도 다음 로직에서 변수 사용하므로, 금액은 설정함.*/
                       WHEN NO_DATA_FOUND THEN
                            IF V_TMP41_AMT_2014 > 0 THEN
                                V_TMP41_AMT := V_TMP41_AMT_2014;
                            ELSE
                              V_TMP41_AMT := 0; /* @VER.2016 공제할 금액 없으므로 후단은 처리 안함.*/
                            END IF;
                  END;

                   /* ③금액이 0보다 크거나 같을 경우 기부액 전액 공제 : 당해 지정기부금(종교) */
                   /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시 복원 */
                  IF V_TMP41_AMT > 0 THEN
                      V_TMP_STEP := 'U04';
...
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */
                  ...
                  /* 당해 지정, 종교기부금 계산시 잔여기부금이 0보다 작은 경우 UPDATE 하지 않는다. 기존 로직 삭제 (@VER.2019_13) */
                  ELSE
                     /* ③금액이 0미만일 경우 (V_TMP41_AMT_2014) 공제 : 당해 지정기부금(종교)    */
                      V_TMP_STEP := 'U05';
                      UPDATE PAYM436
                         SET CNTRIB_GONGAMT = V_TMP41_AMT_2014
                           , CNTRIB_OVERAMT = CNTRIB_GIAMT - V_TMP41_AMT_2014
                       WHERE YY             = IN_YY
                         AND CNTRIB_YY      = IN_YY
                         AND SETT_FG        = V_SETT_FG
                         AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                         AND RPST_PERS_NO   = REC.RPST_PERS_NO
                         AND CNTRIB_TYPE_CD  = 'A032400007'
                         ;
                  --eterain 지정 복원
                ...
                /*시뮬: @VER.2016_8 지정기부금 세액공제대상기부액 종교외,종교 분리 */
                SELECT SUM(DECODE(A.CNTRIB_TYPE_CD, 'A032400006', A.CNTRIB_GONGAMT, 0))
                      ,SUM(DECODE(A.CNTRIB_TYPE_CD, 'A032400007', A.CNTRIB_GONGAMT, 0))
                  INTO V_APNT_CNTRIB40_DUC_OBJ_AMT --지정기부금(40:종교외) 세액공제대상기부금액
                      ,V_APNT_CNTRIB41_DUC_OBJ_AMT --지정기부금(41:종교)  세액공제대상기부금액
                  FROM PAYM436 A --기부금 이월_시뮬레이션
                 WHERE A.YY           = IN_YY
                   AND A.BIZR_DEPT_CD = IN_BIZR_DEPT_CD
                   AND A.SETT_FG      = V_SETT_FG    /* 유의:IN_SETT_FG 시뮬에서는 V_SETT_FG 으로 정산구분 사용 */
                   AND A.RPST_PERS_NO = REC.RPST_PERS_NO
                   AND A.CNTRIB_YY    >= '2014'      /*세액공제:2014년이후 */
                   AND CNTRIB_TYPE_CD IN ('A032400006','A032400007') /*지정(코드:40):종교외, 지정(코드:41):종교*/
                ;


/* ZZODEM */
/*V_OCCR_LOC_NM   := '법정기부금 체크:6';
V_DB_ERROR_CTNT := 'V_APNT_CNTRIB_DUC_OBJ_AMT:'||V_APNT_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_TAXDUC_AMT:'||V_APNT_CNTRIB_TAXDUC_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB40_DUC_OBJ_AMT:'||V_APNT_CNTRIB40_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_FLAW_CNTRIB_DUC_OBJ_AMT:'||V_FLAW_CNTRIB_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB41_DUC_OBJ_AMT:'||V_APNT_CNTRIB41_DUC_OBJ_AMT||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */

                /* @VER.2016_8 지정기부금 세액공제액 종교외,종교 분리 */
                IF V_APNT_CNTRIB_DUC_OBJ_AMT > 0 THEN
                /* 1.지정기부금(40:종교외) 세액공제액 = 전체지정기부금세액공제액 * ( 지정(40:종교외) 대상금액 / 전체지정기부금공제대상액) [비율로 계산] */
                   /* @VER.2017_99 법정+지정(종교외) 2000만원 초과시 세액공제액 보정 */
                   /* @VER.2019_3  고액기부금 기준 금액 변화(2천만원에서 1천만원으로 변경) */
                   IF (V_FLAW_CNTRIB_DUC_OBJ_AMT + V_APNT_CNTRIB40_DUC_OBJ_AMT) > 10000000 THEN
                      IF V_FLAW_CNTRIB_DUC_OBJ_AMT = 0 THEN
                         /* 법정기부금 없이 지정(종교외) 단독으로 2000만원 초과시*/
                         /* @VER.2019_3  고액기부금 기준 금액 변화(2천만원에서 1천만원으로 변경) */
                         V_APNT_CNTRIB40_TAXDUC_AMT := TRUNC(10000000*0.15 + (V_APNT_CNTRIB40_DUC_OBJ_AMT - 10000000) * 0.3) ;
                      ELSE
                         --V_APNT_CNTRIB40_TAXDUC_AMT := TRUNC(V_APNT_CNTRIB_TAXDUC_AMT * ( V_APNT_CNTRIB40_DUC_OBJ_AMT /V_APNT_CNTRIB_DUC_OBJ_AMT));
                         /* 변경된 엑셀파일 참조해서 로직수정 */
                         /* @VER.2019_3  고액기부금 기준 금액 변화(2천만원에서 1천만원으로 변경) */
                         IF (V_FLAW_CNTRIB_DUC_OBJ_AMT > 10000000) THEN
                            /* 법정기부금이 이미 2000만원 초과시 이후 지정기부금은 30% 계산*/
                            V_APNT_CNTRIB40_TAXDUC_AMT := TRUNC(V_APNT_CNTRIB40_DUC_OBJ_AMT * 0.3);
                         ELSE
                            V_APNT_CNTRIB40_TAXDUC_AMT := TRUNC((10000000-V_FLAW_CNTRIB_DUC_OBJ_AMT)*0.15 + (V_APNT_CNTRIB40_DUC_OBJ_AMT-(10000000-V_FLAW_CNTRIB_DUC_OBJ_AMT))* 0.3);
                         END IF;
                      END IF;
                   ELSE
                      /* 합산해도 2000만원 이하시 그냥 15% @VER.2017_99 */
                      /* @VER.2019_3  고액기부금 기준 금액 변화(2천만원에서 1천만원으로 변경) */
                      V_APNT_CNTRIB40_TAXDUC_AMT := TRUNC(V_APNT_CNTRIB40_DUC_OBJ_AMT * 0.15);
                   END IF;

/* ZZODEM */
/*V_OCCR_LOC_NM   := '법정기부금 체크:6-1';
V_DB_ERROR_CTNT := 'V_APNT_CNTRIB41_DUC_OBJ_AMT:'||V_APNT_CNTRIB41_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB41_TAXDUC_AMT:'||V_APNT_CNTRIB41_TAXDUC_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB_TAXDUC_AMT:'||V_APNT_CNTRIB_TAXDUC_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB40_DUC_OBJ_AMT:'||V_APNT_CNTRIB40_DUC_OBJ_AMT||chr(13)||chr(10)||
                   'V_APNT_CNTRIB40_TAXDUC_AMT:'||V_APNT_CNTRIB40_TAXDUC_AMT||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP ); */


                /* 2.지정기부금(41:종교외) 세액공제액 = 전체지정기부금세액공제액 - 지정기부금(40:종교외)세액공제액 */
                   IF V_APNT_CNTRIB41_DUC_OBJ_AMT > 0 THEN
                      V_APNT_CNTRIB41_TAXDUC_AMT := V_APNT_CNTRIB_TAXDUC_AMT - V_APNT_CNTRIB40_TAXDUC_AMT;
                   END IF;
            ...
            ELSE
                ...
                /**[계산:소득공제] (35)기부금(이월분):V_CNTRIB_AMT_CYOV_AMT 2013년이전 이월기부금)update***법정기부금,종교단체외 지정기부금,종교단체 지정기부금 순.************/
                IF V_CNTRIB_AMT_CYOV_AMT = 0 THEN
                   V_TMP_STEP := 'UU2013_0';
                   /* 2013년 이전 기부금(이월분)이 0 일경우 2013년이전 이월분 모두 미공제 처리*/
                   UPDATE PAYM432
...
--                        SELECT GREATEST(V_TMP10_AMT_2014 - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                                ...
                                --SET CNTRIB_GONGAMT = V_TMP10_AMT_2014
                                 SET CNTRIB_GONGAMT = (CNTRIB_GIAMT - CNTRIB_PREAMT)   -- V_TMP_AMT
                                   , CNTRIB_OVERAMT = CASE WHEN REC_10_IWOL_2014.CNTRIB_YY = IN_YY - 6 AND V_TMP_AMT > 0
                                                           ...
                                                           --eterain
                                                           THEN GREATEST((CNTRIB_GIAMT - CNTRIB_PREAMT) - V_TMP_AMT,  V_TMP_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT) )
                                                           --
                ...
                /**[계산:세액공제] 법정기부(법정:10) (당해,2014년이후 이월) update*********************************************************************************************/
                /* 기부금 공제 순서 변경. 2014년 이후 이월금액이 0이하일 경우 모두 미공제 처리 @2019_7*/
                V_TMP10_AMT_2014 :=                 V_TMP_AMT;
                ...
                IF V_TMP_AMT < 0 THEN
                   V_TMP_STEP := 'UU10_0';
                   /* 법정기부금 대상금액(세액공제대상)이 0 일경우 기부년도에 상관없이 모두 미공제 처리 ( 단,2013년 이전기부금은 제외:특별소득공제)*/
                   /* 기부금 공제 순서 변경. 2014년 이후 이월금액이 0이하일 경우 모두 미공제 처리 @2019_7*/
                   /*
                    UPDATE PAYM432
                      SET CNTRIB_GONGAMT = 0
                ...
                ELSE
                  ...
                  BEGIN
                    ...
                    --eterain
                    --SELECT GREATEST(V_TMP10_AMT_2014 - CNTRIB_GIAMT, 0)
                    SELECT GREATEST(V_TMP_AMT - CNTRIB_GIAMT, 0)
                    --
                   ...
                   EXCEPTION
                        ...
                        --eterain3 WHEN NO_DATA_FOUND THEN V_TMP10_AMT := V_TMP10_AMT_2014;
                        WHEN NO_DATA_FOUND THEN V_TMP10_AMT := V_TMP_AMT;
                            /* 기부금 공제 순서 변경. 2014년 이후 이월금액으로 처리 @2019_7*/
                   END;
                    ...
                    /* ①금액이 0이하일 경우 법정기부공제대상금액 공제 :당해 법정기부금 */
                      ...
                      UPDATE PAYM432                         
                         SET CNTRIB_GONGAMT = V_TMP_AMT
                           ...
                           --eterain
                           --, CNTRIB_OVERAMT = CNTRIB_GIAMT - V_TMP_AMT
                           ,CNTRIB_OVERAMT = GREATEST(CNTRIB_GIAMT - V_TMP_AMT,  V_TMP_AMT - CNTRIB_GIAMT)
                           --                           
                   ...
                   UPDATE PAYM432
                     SET CNTRIB_GONGAMT = 0
                       , CNTRIB_OVERAMT = CASE WHEN CNTRIB_YY = IN_YY - 10
                                               ...
                                               --eterain    
                                               --ELSE CNTRIB_GIAMT - CNTRIB_PREAMT
                                               ELSE GREATEST(CNTRIB_GIAMT - CNTRIB_PREAMT,  CNTRIB_PREAMT - CNTRIB_GIAMT)
                                               --                                               
                      ...
                      LOOP
                       ...
                       /* ②:지정기부공제대상금액 - 당해 지정기부금(종교외) - 2014년이후 이월 지정기부금(종교외).. */
                        ...
                        END;
                          ...
                          /*  ②금액이 0보다 크거나 같을 경우 기부액 전액 공제 : 2014년이후 이월 지정기부금(종교외) */
                          /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시 복원 */
                          IF V_TMP40_AMT_2014 > 0 THEN
                              V_TMP_STEP := 'U40_2014_1';
                              UPDATE PAYM432
                              ...
                              IF V_TMP_CNT < V_REC_CNT  THEN
                                  SELECT CNTRIB_GIAMT - CNTRIB_PREAMT
                                   ...
                                   WHERE YY             = IN_YY
                                     ...
                                     ;
                              ELSIF V_TMP_CNT = V_REC_CNT THEN
                                /* 마지막 자료일 경우 지정_종교 이월용 기준금액으로 설정.*/
                                   SELECT SUM(CNTRIB_GONGAMT)
                                     INTO V_TMP40_AMT_2014
                                     FROM PAYM432
                                    WHERE YY             = IN_YY
                              ...
                              END IF;
                          ELSE
                             /* ②금액이 0미만일 경우 (V_TMP40_AMT) 공제 : 2014년이후 이월 지정기부금(종교외) */
                              V_TMP_STEP := 'U40_2014_2';
                              UPDATE PAYM432
                                 SET CNTRIB_GONGAMT = V_TMP40_AMT
                                   , CNTRIB_OVERAMT = CASE WHEN REC_40_IWOL_2014.CNTRIB_YY = IN_YY - 10
...
----------------------------------
                  ...
                  /* ①금액이 0보다 크거나 같을 경우 기부액 전액 공제 : 당해 지정기부금(종교외) */
                  /* 전액공제를 > 0 에서 >= 0 으로 변경(@VER.2019_10). 다시 복원 */
                  IF V_TMP40_AMT > 0 THEN
                      V_TMP_STEP := 'UU40_1';
                      UPDATE PAYM432
...
------------------------
                      ...
                      BEGIN
                        ...
                        LOOP
                         ...
                         /* ④:지정기부공제대상금액 -  지정기부금 종교외(당해+이월) - 당해 지정기부금(종교) - 2014년이후 이월 지정기부금(종교)*/
                          SELECT GREATEST(V_TMP41_AMT - (CNTRIB_GIAMT - CNTRIB_PREAMT), 0)
                            INTO V_TMP41_AMT_2014
                            FROM PAYM432
                           WHERE YY             = IN_YY
...
----------------------------------
                  ...
                  EXCEPTION
                       ...
                       WHEN NO_DATA_FOUND THEN
                            IF V_TMP41_AMT_2014 > 0 THEN
                                V_TMP41_AMT := V_TMP41_AMT_2014;
                            ELSE
                              V_TMP41_AMT := 0; /* @VER.2016 이미 공제할 금액이 더이상 없기 때문에 후단은 처리 안함.*/
                            END IF;
                  END;
                ...
                /*실제연말정산계산: @VER.2016_8 지정기부금 세액공제대상기부액 종교외,종교 분리 */
                SELECT SUM(DECODE(A.CNTRIB_TYPE_CD, 'A032400006', A.CNTRIB_GONGAMT, 0))
                      ,SUM(DECODE(A.CNTRIB_TYPE_CD, 'A032400007', A.CNTRIB_GONGAMT, 0))
                  INTO V_APNT_CNTRIB40_DUC_OBJ_AMT --지정기부금(40:종교외) 세액공제대상기부금액
                      ,V_APNT_CNTRIB41_DUC_OBJ_AMT --지정기부금(41:종교)  세액공제대상기부금액
                  FROM PAYM432 A --기부금 이월
                 WHERE A.YY           = IN_YY
                   AND A.YRETXA_SEQ   = IN_YRETXA_SEQ /*@VER.2017_0*/
                   AND A.BIZR_DEPT_CD = IN_BIZR_DEPT_CD
...
--
--                --지정기부금세액공제액
--                IF (V_FLAW_CNTRIB_DUC_OBJ_AMT + V_APNT_CNTRIB_DUC_OBJ_AMT) > 30000000 THEN
--                    V_APNT_CNTRIB_TAXDUC_AMT := TRUNC((V_TMP_AMT - 30000000) * 0.25 + 30000000 * 0.15 - V_FLAW_CNTRIB_TAXDUC_AMT);
--                ELSE
--                    V_APNT_CNTRIB_TAXDUC_AMT := TRUNC(V_TMP_AMT * 0.15 - V_FLAW_CNTRIB_TAXDUC_AMT);
--                END IF;
--
--                -- 산출세액 차감
--                SELECT SF_SETT_CHAGAM_CAL(V_CAL_TDUC_TEMP_AMT, V_APNT_CNTRIB_TAXDUC_AMT,1),
--                       SF_SETT_CHAGAM_CAL(V_CAL_TDUC_TEMP_AMT, V_APNT_CNTRIB_TAXDUC_AMT,2)
--                INTO V_CAL_TDUC_TEMP_AMT, V_APNT_CNTRIB_TAXDUC_AMT
--                FROM DUAL;
--
--                -- 세액공제 계
--                V_TDUC_DUC_TT_AMT := V_TDUC_DUC_TT_AMT + V_APNT_CNTRIB_TAXDUC_AMT;
--
--            ELSE
--                V_APNT_CNTRIB_DUC_OBJ_AMT := 0;
--
...
--                DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_TDUC_DUC_AMT = '||TO_CHAR(V_LABOR_EARN_TDUC_DUC_AMT) );
--                DBMS_OUTPUT.PUT_LINE('V_BF_SITE_RTXLW_TAX = '||TO_CHAR(V_BF_SITE_RTXLW_TAX) );
--                DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_TDUC_DUC_AMT = '||TO_CHAR(V_LABOR_EARN_TDUC_DUC_AMT) );


--                --감면비율을 계산함...  ==> 가. MIN( 가.번 계산값, 500000) × (1-감면급여비율)
--                IF ( V_LABOR_EARN_TT_SALY_AMT > 0 AND V_BF_SITE_STXLW_AMT > 0) THEN
--                    V_LABOR_EARN_TDUC_DUC_AMT := V_LABOR_EARN_TDUC_DUC_AMT * (1.0 - V_BF_SITE_STXLW_AMT/V_LABOR_EARN_TT_SALY_AMT);
----                    DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_TDUC_DUC_AMT = '||TO_CHAR(V_LABOR_EARN_TDUC_DUC_AMT) );
----                    DBMS_OUTPUT.PUT_LINE('V_BF_SITE_STXLW_TAX = '||TO_CHAR(V_BF_SITE_STXLW_AMT) );
----                    DBMS_OUTPUT.PUT_LINE('V_LABOR_EARN_TT_SALY_AMT = '||TO_CHAR(V_LABOR_EARN_TT_SALY_AMT) );
----                    DBMS_OUTPUT.PUT_LINE('V_BF_SITE_STXLW_TAX/V_LABOR_EARN_TT_SALY_AMT = '||TO_CHAR(V_BF_SITE_STXLW_TAX/V_LABOR_EARN_TT_SALY_AMT) );
--                END IF;
--
--                --DBMS_OUTPUT.PUT_LINE('V_CAL_TDUC_TEMP_AMT = '||TO_CHAR(V_CAL_TDUC_TEMP_AMT) );
--
--                SELECT SF_SETT_CHAGAM_CAL(V_CAL_TDUC_TEMP_AMT,V_LABOR_EARN_TDUC_DUC_AMT,1),
--                       SF_SETT_CHAGAM_CAL(V_CAL_TDUC_TEMP_AMT,V_LABOR_EARN_TDUC_DUC_AMT,2)
--                  INTO V_CAL_TDUC_TEMP_AMT, V_LABOR_EARN_TDUC_DUC_AMT
...
V_OCCR_LOC_NM := '외국인 체크';
V_DB_ERROR_CTNT := 'V_LABOR_EARN_TT_SALY_AMT = ' || V_LABOR_EARN_TT_SALY_AMT ||chr(13)||chr(10);
SP_SSTM056_CREA(V_DB_PGM_ID, V_OCCR_LOC_NM, SQLCODE, V_DB_ERROR_CTNT, IN_INPT_ID , IN_INPT_IP );
           ...
           IF V_TMP_CNT > 0 THEN           
                DELETE FROM PAYM432
                 WHERE YY             = IN_YY
                   ...
                   AND BIZR_DEPT_CD   = IN_BIZR_DEPT_CD
                   AND SETT_FG        = V_SETT_FG 
                   AND RPST_PERS_NO IN ( SELECT C.RPST_PERS_NO 
                                           FROM PAYM410_2019TMP C
                                          WHERE C.YY         = IN_YY
                                            AND C.YRETXA_SEQ = IN_YRETXA_SEQ
                                            AND C.SETT_FG    = V_SETT_FG
                ...
                ;
                INSERT INTO PAYM432 ( BIZR_DEPT_CD,YY,SETT_FG,RPST_PERS_NO,CNTRIB_YY,CNTRIB_TYPE_CD,CNTRIB_GIAMT,CNTRIB_PREAMT,CNTRIB_GONGAMT,CNTRIB_DESTAMT,CNTRIB_OVERAMT,INPT_ID,INPT_DTTM,INPT_IP,MOD_ID,MOD_DTTM,MOD_IP,YRETXA_SEQ )
                SELECT BIZR_DEPT_CD,YY,SETT_FG,RPST_PERS_NO,CNTRIB_YY,CNTRIB_TYPE_CD,CNTRIB_GIAMT,CNTRIB_PREAMT,CNTRIB_GONGAMT,CNTRIB_DESTAMT,CNTRIB_OVERAMT,INPT_ID,INPT_DTTM,INPT_IP,MOD_ID,MOD_DTTM,MOD_IP,YRETXA_SEQ  
                FROM PAYM432_2019TMP C
                WHERE C.YY         = IN_YY
                AND C.YRETXA_SEQ = IN_YRETXA_SEQ
                AND C.SETT_FG    = V_SETT_FG
                AND C.BIZR_DEPT_CD = IN_BIZR_DEPT_CD
                ;
                INSERT INTO PAYM410 ( BIZR_DEPT_CD,YRETXA_SEQ,YY,SETT_FG,RPST_PERS_NO,RPRT_YYMM,POSI_BREU_CD,POSI_DEPT_CD,KOR_NM,RES_NO,NATI_FG,RSD_FG,RSD_NATI_FG,LABOR_SCHLS_FG,STTS_FG,BIZTP_FG,WKSP_FG,LVLPT_FG,WKGD_CD,STEP_FG,CWK_YCNT,HOUSEHOLDER_YN,REDC_FR_DT,REDC_TO_DT,MA_WK_SALY_AMT,MA_WK_BONUS_AMT,MA_DETM_BONUS_AMT,MA_WK_TT_AMT,BF_WK_FIRM_NM,BF_WK_FR_DT_1,BF_WK_TO_DT_1,BF_REDC_FR_DT_1,BF_REDC_TO_DT_1,BF_WK_NO,BF_WK_SALY_AMT,BF_WK_BONUS_AMT,BF_WK_DETM_BONUS_AMT,BF_WK_REDC_TAX_AMT1,BF_WK_FIRM_2_NM,BF_WK_FR_DT_2,BF_WK_TO_DT_2,BF_WK_NO_2,BF_WK_SALY_2_AMT,BF_WK_BONUS_2_AMT,BF_WK_DETM_BONUS_2_AMT,BF_WK_REDC_TAX_AMT2,BF_WK_FIRM_3_NM,BF_WK_FR_DT_3,BF_WK_TO_DT_3,BF_REDC_FR_DT_3,BF_REDC_TO_DT_3,BF_WK_NO_3,BF_WK_SALY_3_AMT,BF_WK_BONUS_3_AMT,BF_WK_DETM_BONUS_3_AMT,BF_WK_REDC_TAX_AMT3,BF_WK_REDC_TAX_TT_AMT,BF_WK_TT_AMT,SALY_TT_AMT,BONUS_TT_AMT,DETM_BONUS_TT_AMT,TAX_TT,FRN_NOTAX_AMT,DELAY_NOTAX_AMT,RECH_NOTAX_AMT,CARE_NOTAX_AMT,LABOR_SCHLS_NOTAX_AMT,ETC_NOTAX_AMT,NOTAX_TT_AMT,REDC_EARN_AMT,LABOR_EARN_AMT,LABOR_EARN_DUC_AMT,SLF_DUC_AMT,WIFE_DUC_AMT,SPRT_PSN_CNT,SPRT_DUC,RSPT_DUC_CNT,PATH_DUC_AMT,HINDR_CNT,HIND_DUC_AMT,WOMN_DUC_AMT,BRED_CNT,BRED_DUC_AMT,CHDBIRTH_DUC_CNT,CHDBIRTH_DUC_AMT,MULT_CHILD_ADD_DUC_CNT,MULT_CHILD_ADD_DUC_AMT,SINGLE_PARENT_DUC_AMT,NPN_DUC_AMT,PUBPERS_PENS_DUC_AMT,NPN_INSU_AMT,MILITARY_PENS_DUC_AMT,RETI_SCI_PENS_EARN_DUC_AMT,RETI_PENS_EARN_DUC_AMT,SPCL_DUC_INSU_AMT,SPCL_DUC_HINS_AMT,SPCL_DUC_EINS_AMT,SPCL_DUC_GUAR_INSU_AMT,SPCL_DUC_HANDICAP_INSU_AMT,SPCL_DUC_HFE_AMT,SPCL_DUC_HANDICAP_HFE_AMT,SPCL_DUC_ETC_HFE_AMT,SPCL_DUC_ED_AMT,SPCL_DUC_HANDICAP_EDU_AMT,SPCL_DUC_ETC_EDU_AMT,SPCL_DUC_HSFND_AMT,SPCL_DUC_HOUS_LOAMT_AMT1,SPCL_DUC_HOUS_LOAMT_AMT2,SPCL_DUC_MNRT_AMT,SPCL_DUC_HOUS_ITT_AMT,SPCL_DUC_HOUS_ITT_AMT1,SPCL_DUC_HOUS_ITT_AMT2,SPCL_DUC_HOUS_ITT_AMT3,SPCL_DUC_HOUS_ITT_AMT4,SPCL_DUC_HOUS_ITT_AMT5,SPCL_DUC_HOUS_ITT_AMT6,SPCL_DUC_HOUS_ITT_AMT7,SPCL_DUC_HOUS_ITT_AMT8,SPCL_DUC_HOUS_ITT_AMT9,SPCL_DUC_CNTRIB_AMT,SPCL_DUC_MAGE_MV_AMT,STAD_DUC_AMT,SBTR_EARN_AMT,PERS_PESN_SAV_DUC_AMT,PESN_SAV_DUC_AMT,DUC_INME_EARN_DUC_AMT,SUBSCRP_SAV_DUC_AMT,HOUS_SUBSCRP_GNR_SAV_DUC_AMT,LNTM_HSSV_DUC_AMT,LABORR_HSSV_DUC_AMT,HOUS_SAV_DUC_AMT,ICOMP_DUC_AMT,DEBIT_CARD_DUC_AMT,CREDIT_CARD_DUC_AMT,OSC_EARN_DUC_AMT,LSTCS_SAV_DUC_AMT,LSTCS_SAV_DUC_AMT1,LSTCS_SAV_DUC_AMT2,LSTCS_SAV_DUC_AMT3,TXSTD_AMT,CAL_TAX,ITXLW,STXLW,RTXLW,RTXLW_OBJ_AMT,RTXLW_CURR_REDC_AMT,RTXLW_ALD_REDC_AMT,REDC_TAX_TT,LABOR_EARN_TDUC_AMT,TXPYAS_TDUC_AMT,LESE_LOAMT_TDUC_AMT,CNTRIB_POLITICS_TDUC_AMT,FRNPAI_TDUC_AMT,TDUC_TT,DETM_EARN_AMT,DETM_FMTAX_AMT,DETM_IHTAX_AMT,DETM_TT_AMT,ALD_BF_EARN_AMT,ALD_BF_FMTAX_AMT,ALD_BF_IHTAX_AMT,ALD_BF_TT_AMT,ALD_MA_EARN_AMT,ALD_MA_FMTAX_AMT,ALD_MA_IHTAX_AMT,ALD_MA_TT_AMT,PAID_SPCLEX_TAX,ALD_SBTR_EARN_AMT,ALD_SBTR_FMTAX_AMT,ALD_SBTR_IHTAX_AMT,ALD_SBTR_TT_AMT,RPRT_YN,FLAW_CNTRIB_AMT,POLITICS_TRSR_CNTRIB_AMT,POLITICS_TRSR_AMT,THYR_FLAW_CNTRIB_AMT,SPCLEX_CNTRIB_AMT,APNT_CNTRIB_AMT,RELI_GRP_APNT_CNTRIB_AMT,CNTRIB_AMT_OSC_SOCT,FLAW_CNTRIB_AMT_CYOV_AMT,SPCLEX_CNTRIB_AMT_CYOV_AMT,RELI_OUT_CNTRIB_AMT_CYOV_AMT,RELI_GRP_CNTRIB_AMT_CYOV_AMT,LFSTS_ITT_RFND_AMT,SPCL_DUC_GNR_LMT_AMT,SPCL_DUC_GNR_LMT_EXCE_AMT,CURR_WK_FR_DT,CURR_WK_TO_DT,REMK,BF_DELAY_NOTAX_AMT_1,BF_CARE_NOTAX_AMT_1,BF_ETC_NOTAX_AMT_1,BF_APNT_NOTAX_AMT_1,BF_NOTAX_TT_AMT_1,BF_DELAY_NOTAX_AMT_2,BF_CARE_NOTAX_AMT_2,BF_ETC_NOTAX_AMT_2,BF_APNT_NOTAX_AMT_2,BF_NOTAX_TT_AMT_2,BF_DELAY_NOTAX_AMT_3,BF_CARE_NOTAX_AMT_3,BF_ETC_NOTAX_AMT_3,BF_APNT_NOTAX_AMT_3,BF_NOTAX_TT_AMT_3,BF_RECH_NOTAX_AMT_1,BF_RECH_NOTAX_AMT_2,BF_RECH_NOTAX_AMT_3,FORE_TAX_RATE_YN,INPT_ID,INPT_DTTM,INPT_IP,MOD_ID,MOD_DTTM,MOD_IP,BF_REDC_FR_DT_2,BF_REDC_TO_DT_2,BASE_DUC_CHILD_CNT,CHILD_TAXDUC_AMT,CNTRIB_AMT_CYOV_AMT,OSC_CNTRB_AMT,OSC_CNTRIB_AMT,LNTM_GATHER_INVST_SEC_SAV_AMT,SCI_TECH_PSN_PENS_DUC_OBJ_AMT,SCI_TECH_PSN_PENS_TAXDUC_AMT,RETI_PENS_DUC_OBJ_AMT,RETI_PENS_TAXDUC_AMT,PNSV_DUC_OBJ_AMT,PNSV_TAXDUC_AMT,GUARQL_INSU_DUC_OBJ_AMT,GUARQL_INSU_TAXDUC_AMT,HFE_DUC_OBJ_AMT,HFE_TAXDUC_AMT,EDAMT_DUC_OBJ_AMT,EDAMT_TAXDUC_AMT,POLITICS_LMT_BLW_DUC_OBJ

_AMT,POLITICS_LMT_BLW_TAXDUC_AMT,POLITICS_LMT_EXCE_DUC_OBJ_AMT,POLITICS_LMT_EXCE_TAXDUC_AMT,FLAW
_CNTRIB_DUC_OBJ_AMT,FLAW_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_DUC_OBJ_AMT,APNT_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_RELI_OUT_OBJ_AMT,APNT_CNTRIB_RELI_OUT_DUC_AMT,APNT_CNTRIB_RELI_OBJ_AMT,APNT_CNTRIB_RELI_DUC_AMT,STAD_TAXDUC_AMT,MNRT_TAXDUC_AMT,DSPSN_GUARQL_INSU_DUC_OBJ_AMT,DSPSN_GUARQL_INSU_TAXDUC_AMT,YRETXA_PART_FXD_YN,DUTY_INVENT_CMPS_AMT_NOTAX,NPN_DUC_OBJ_AMT,NPN_INSU_DUC_OBJ_AMT,PUBPERS_PENS_DUC_OBJ_AMT,MILITARY_PENS_DUC_OBJ_AMT,SPCL_DUC_HINS_DUC_OBJ_AMT,SPCL_DUC_EINS_DUC_OBJ_AMT,STOCK_BUY_CHOICE_PROFIT_AMT,SMBIZ_BONUS_TAX )
                SELECT BIZR_DEPT_CD,YRETXA_SEQ,YY,SETT_FG,RPST_PERS_NO,RPRT_YYMM,POSI_BREU_CD,POSI_DEPT_CD,KOR_NM,RES_NO,NATI_FG,RSD_FG,RSD_NATI_FG,LABOR_SCHLS_FG,STTS_FG,BIZTP_FG,WKSP_FG,LVLPT_FG,WKGD_CD,STEP_FG,CWK_YCNT,HOUSEHOLDER_YN,REDC_FR_DT,REDC_TO_DT,MA_WK_SALY_AMT,MA_WK_BONUS_AMT,MA_DETM_BONUS_AMT,MA_WK_TT_AMT,BF_WK_FIRM_NM,BF_WK_FR_DT_1,BF_WK_TO_DT_1,BF_REDC_FR_DT_1,BF_REDC_TO_DT_1,BF_WK_NO,BF_WK_SALY_AMT,BF_WK_BONUS_AMT,BF_WK_DETM_BONUS_AMT,BF_WK_REDC_TAX_AMT1,BF_WK_FIRM_2_NM,BF_WK_FR_DT_2,BF_WK_TO_DT_2,BF_WK_NO_2,BF_WK_SALY_2_AMT,BF_WK_BONUS_2_AMT,BF_WK_DETM_BONUS_2_AMT,BF_WK_REDC_TAX_AMT2,BF_WK_FIRM_3_NM,BF_WK_FR_DT_3,BF_WK_TO_DT_3,BF_REDC_FR_DT_3,BF_REDC_TO_DT_3,BF_WK_NO_3,BF_WK_SALY_3_AMT,BF_WK_BONUS_3_AMT,BF_WK_DETM_BONUS_3_AMT,BF_WK_REDC_TAX_AMT3,BF_WK_REDC_TAX_TT_AMT,BF_WK_TT_AMT,SALY_TT_AMT,BONUS_TT_AMT,DETM_BONUS_TT_AMT,TAX_TT,FRN_NOTAX_AMT,DELAY_NOTAX_AMT,RECH_NOTAX_AMT,CARE_NOTAX_AMT,LABOR_SCHLS_NOTAX_AMT,ETC_NOTAX_AMT,NOTAX_TT_AMT,REDC_EARN_AMT,LABOR_EARN_AMT,LABOR_EARN_DUC_AMT,SLF_DUC_AMT,WIFE_DUC_AMT,SPRT_PSN_CNT,SPRT_DUC,RSPT_DUC_CNT,PATH_DUC_AMT,HINDR_CNT,HIND_DUC_AMT,WOMN_DUC_AMT,BRED_CNT,BRED_DUC_AMT,CHDBIRTH_DUC_CNT,CHDBIRTH_DUC_AMT,MULT_CHILD_ADD_DUC_CNT,MULT_CHILD_ADD_DUC_AMT,SINGLE_PARENT_DUC_AMT,NPN_DUC_AMT,PUBPERS_PENS_DUC_AMT,NPN_INSU_AMT,MILITARY_PENS_DUC_AMT,RETI_SCI_PENS_EARN_DUC_AMT,RETI_PENS_EARN_DUC_AMT,SPCL_DUC_INSU_AMT,SPCL_DUC_HINS_AMT,SPCL_DUC_EINS_AMT,SPCL_DUC_GUAR_INSU_AMT,SPCL_DUC_HANDICAP_INSU_AMT,SPCL_DUC_HFE_AMT,SPCL_DUC_HANDICAP_HFE_AMT,SPCL_DUC_ETC_HFE_AMT,SPCL_DUC_ED_AMT,SPCL_DUC_HANDICAP_EDU_AMT,SPCL_DUC_ETC_EDU_AMT,SPCL_DUC_HSFND_AMT,SPCL_DUC_HOUS_LOAMT_AMT1,SPCL_DUC_HOUS_LOAMT_AMT2,SPCL_DUC_MNRT_AMT,SPCL_DUC_HOUS_ITT_AMT,SPCL_DUC_HOUS_ITT_AMT1,SPCL_DUC_HOUS_ITT_AMT2,SPCL_DUC_HOUS_ITT_AMT3,SPCL_DUC_HOUS_ITT_AMT4,SPCL_DUC_HOUS_ITT_AMT5,SPCL_DUC_HOUS_ITT_AMT6,SPCL_DUC_HOUS_ITT_AMT7,SPCL_DUC_HOUS_ITT_AMT8,SPCL_DUC_HOUS_ITT_AMT9,SPCL_DUC_CNTRIB_AMT,SPCL_DUC_MAGE_MV_AMT,STAD_DUC_AMT,SBTR_EARN_AMT,PERS_PESN_SAV_DUC_AMT,PESN_SAV_DUC_AMT,DUC_INME_EARN_DUC_AMT,SUBSCRP_SAV_DUC_AMT,HOUS_SUBSCRP_GNR_SAV_DUC_AMT,LNTM_HSSV_DUC_AMT,LABORR_HSSV_DUC_AMT,HOUS_SAV_DUC_AMT,ICOMP_DUC_AMT,DEBIT_CARD_DUC_AMT,CREDIT_CARD_DUC_AMT,OSC_EARN_DUC_AMT,LSTCS_SAV_DUC_AMT,LSTCS_SAV_DUC_AMT1,LSTCS_SAV_DUC_AMT2,LSTCS_SAV_DUC_AMT3,TXSTD_AMT,CAL_TAX,ITXLW,STXLW,RTXLW,RTXLW_OBJ_AMT,RTXLW_CURR_REDC_AMT,RTXLW_ALD_REDC_AMT,REDC_TAX_TT,LABOR_EARN_TDUC_AMT,TXPYAS_TDUC_AMT,LESE_LOAMT_TDUC_AMT,CNTRIB_POLITICS_TDUC_AMT,FRNPAI_TDUC_AMT,TDUC_TT,DETM_EARN_AMT,DETM_FMTAX_AMT,DETM_IHTAX_AMT,DETM_TT_AMT,ALD_BF_EARN_AMT,ALD_BF_FMTAX_AMT,ALD_BF_IHTAX_AMT,ALD_BF_TT_AMT,ALD_MA_EARN_AMT,ALD_MA_FMTAX_AMT,ALD_MA_IHTAX_AMT,ALD_MA_TT_AMT,PAID_SPCLEX_TAX,ALD_SBTR_EARN_AMT,ALD_SBTR_FMTAX_AMT,ALD_SBTR_IHTAX_AMT,ALD_SBTR_TT_AMT,RPRT_YN,FLAW_CNTRIB_AMT,POLITICS_TRSR_CNTRIB_AMT,POLITICS_TRSR_AMT,THYR_FLAW_CNTRIB_AMT,SPCLEX_CNTRIB_AMT,APNT_CNTRIB_AMT,RELI_GRP_APNT_CNTRIB_AMT,CNTRIB_AMT_OSC_SOCT,FLAW_CNTRIB_AMT_CYOV_AMT,SPCLEX_CNTRIB_AMT_CYOV_AMT,RELI_OUT_CNTRIB_AMT_CYOV_AMT,RELI_GRP_CNTRIB_AMT_CYOV_AMT,LFSTS_ITT_RFND_AMT,SPCL_DUC_GNR_LMT_AMT,SPCL_DUC_GNR_LMT_EXCE_AMT,CURR_WK_FR_DT,CURR_WK_TO_DT,REMK,BF_DELAY_NOTAX_AMT_1,BF_CARE_NOTAX_AMT_1,BF_ETC_NOTAX_AMT_1,BF_APNT_NOTAX_AMT_1,BF_NOTAX_TT_AMT_1,BF_DELAY_NOTAX_AMT_2,BF_CARE_NOTAX_AMT_2,BF_ETC_NOTAX_AMT_2,BF_APNT_NOTAX_AMT_2,BF_NOTAX_TT_AMT_2,BF_DELAY_NOTAX_AMT_3,BF_CARE_NOTAX_AMT_3,BF_ETC_NOTAX_AMT_3,BF_APNT_NOTAX_AMT_3,BF_NOTAX_TT_AMT_3,BF_RECH_NOTAX_AMT_1,BF_RECH_NOTAX_AMT_2,BF_RECH_NOTAX_AMT_3,FORE_TAX_RATE_YN,INPT_ID,INPT_DTTM,INPT_IP,MOD_ID,MOD_DTTM,MOD_IP,BF_REDC_FR_DT_2,BF_REDC_TO_DT_2,BASE_DUC_CHILD_CNT,CHILD_TAXDUC_AMT,CNTRIB_AMT_CYOV_AMT,OSC_CNTRB_AMT,OSC_CNTRIB_AMT,LNTM_GATHER_INVST_SEC_SAV_AMT,SCI_TECH_PSN_PENS_DUC_OBJ_AMT,SCI_TECH_PSN_PENS_TAXDUC_AMT,RETI_PENS_DUC_OBJ_AMT,RETI_PENS_TAXDUC_AMT,PNSV_DUC_OBJ_AMT,PNSV_TAXDUC_AMT,GUARQL_INSU_DUC_OBJ_AMT,GUARQL_INSU_TAXDUC_AMT,HFE_DUC_OBJ_AMT,HFE_TAXDUC_AMT,EDAMT_DUC_OBJ_AMT,EDAMT_TAXDUC_AMT,POLITICS_LMT_BLW_DUC_OBJ_AMT,POLITICS_L

MT_BLW_TAXDUC_AMT,POLITICS_LMT_EXCE_DUC_OBJ_AMT,POLITICS_LMT_EXCE_TAXDUC_AMT,FLAW_CNTRIB_DUC_OBJ
_AMT,FLAW_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_DUC_OBJ_AMT,APNT_CNTRIB_TAXDUC_AMT,APNT_CNTRIB_RELI_OUT_OBJ_AMT,APNT_CNTRIB_RELI_OUT_DUC_AMT,APNT_CNTRIB_RELI_OBJ_AMT,APNT_CNTRIB_RELI_DUC_AMT,STAD_TAXDUC_AMT,MNRT_TAXDUC_AMT,DSPSN_GUARQL_INSU_DUC_OBJ_AMT,DSPSN_GUARQL_INSU_TAXDUC_AMT,YRETXA_PART_FXD_YN,DUTY_INVENT_CMPS_AMT_NOTAX,NPN_DUC_OBJ_AMT,NPN_INSU_DUC_OBJ_AMT,PUBPERS_PENS_DUC_OBJ_AMT,MILITARY_PENS_DUC_OBJ_AMT,SPCL_DUC_HINS_DUC_OBJ_AMT,SPCL_DUC_EINS_DUC_OBJ_AMT,STOCK_BUY_CHOICE_PROFIT_AMT,SMBIZ_BONUS_TAX   
                FROM PAYM410_2019TMP C
                WHERE C.YY         = IN_YY
                AND C.YRETXA_SEQ = IN_YRETXA_SEQ
                AND C.SETT_FG    = V_SETT_FG
           ...
           EXCEPTION 
               WHEN OTHERS THEN
                   OUT_RTN := 0;
                   OUT_MSG := '연말정산 2019TMP CREATE ( SQLCODE : '||SQLCODE || ':' || SQLERRM || ')';
                   RETURN;
        END;
        --
        -- 2020.02.11 : 2019정산년도만 해당
