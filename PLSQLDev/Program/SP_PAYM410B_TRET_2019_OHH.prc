...
V_CNTRIB40_PRE_DUC_OBJ_AMT             NUMBER(15) := 0;      --지정기부(종교외)공제대상금액 16~18
V_CNTRIB40_PRE_TAXDUC_AMT              NUMBER(15) := 0;      --지정기부(종교외)세액공제액 16~18
...
V_CNTRIB41_PRE_TAXDUC_AMT              NUMBER(15) := 0;      --지정기부(종교)세액공제액 16~18
...
-- richardoh 기부금 세액공제 시작 법정 시작    
             -- 14,15 이월금액 
                 IF V_CNTRIB_10_PPRE_SUM_AMT > 0 THEN     
                    --법정공제대상금액한도 V_LMT_CNTRIB_10_AMT
                    V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT );
                    V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_PPRE_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제 대상금액
                      IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)
                          ...
                          V_FLAW_CNTRIB_PPRE_TAXDUC_AMT      := V_CNTRIB_UP_AMT + V_CNTRIB_DOWN_AMT;
                      END IF;
                    ...
                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PPRE_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_FLAW_CNTRIB_PPRE_TAXDUC_AMT := V_FLAW_CNTRIB_PPRE_TAXDUC_AMT;
                 ...
                 IF V_CNTRIB_10_PRE_SUM_AMT > 0 THEN     
                    ...
                    V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_PRE_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제 대상금액
                      IF V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT + V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                              IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT > 20000000 THEN --직전 누적이 2천 넘을때
                    ...
                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_PRE_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_FLAW_CNTRIB_PRE_TAXDUC_AMT := V_FLAW_CNTRIB_PRE_TAXDUC_AMT;
             ...
             -- 2019 당해 금액 
                 IF V_CNTRIB_10_YEAR_SUM_AMT > 0 THEN     
                    --법정공제대상금액한도 V_LMT_CNTRIB_10_AMT
                    V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT - V_CNTRIB_10_PPRE_SUM_AMT - V_CNTRIB_10_PRE_SUM_AMT);

DBMS_OUTPUT.PUT_LINE('.            eterain법정  2019        한도금액  : V_LMT_CNTRIB_10_AMT     :=GREATEST(0,  V_LMT_CNTRIB_10_AMT - V_CNTRIB_10_PPRE_SUM_AMT - V_CNTRIB_10_PRE_SUM_AMT)');
DBMS_OUTPUT.PUT_LINE('.            eterain법정  2019        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain법정  2019        한도금액  V_CNTRIB_10_PPRE_SUM_AMT : ' || V_CNTRIB_10_PPRE_SUM_AMT);
DBMS_OUTPUT.PUT_LINE('.            eterain법정  2019        한도금액  V_CNTRIB_10_PRE_SUM_AMT  : ' || V_CNTRIB_10_PRE_SUM_AMT);

                    V_FLAW_CNTRIB_DUC_OBJ_AMT       := LEAST(  V_CNTRIB_10_YEAR_SUM_AMT    , V_LMT_CNTRIB_10_AMT       ); --공제 대상금액
                      IF V_FLAW_CNTRIB_DUC_OBJ_AMT > 0 THEN
                          V_LMT_CNTRIB_SUM_AMT      := V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT + V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT + V_FLAW_CNTRIB_DUC_OBJ_AMT;--누적 공제금액 (법정,우리,지정 공제대상  누계액)

                              IF V_FLAW_CNTRIB_PPRE_DUC_OBJ_AMT + V_FLAW_CNTRIB_PRE_DUC_OBJ_AMT > 10000000 THEN --직전 누적이 1천 넘을때
                      ...
                      END IF;
                        ...
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  2019        입력금액  : ' || V_CNTRIB_10_YEAR_SUM_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  2019        한도금액  : ' || V_LMT_CNTRIB_10_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  2019    공제대상금액  : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.            @@법정  2019        공제금액  : ' || V_FLAW_CNTRIB_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액                 한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
                    ...
                    --###################### 법정기부금 ############################
                          IF (V_CAL_TDUC_TEMP_AMT - V_FLAW_CNTRIB_TAXDUC_AMT) >= 0 THEN  --법정기부금
                             V_FLAW_CNTRIB_TAXDUC_AMT := V_FLAW_CNTRIB_TAXDUC_AMT;
                    ...
                    --################################################################
                        ...
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  2019 공제대상금액  : ' || V_FLAW_CNTRIB_DUC_OBJ_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리@@법정  2019     공제금액  : ' || V_FLAW_CNTRIB_TAXDUC_AMT);
                        DBMS_OUTPUT.PUT_LINE('.           세액처리                 한도금액  : ' || V_CAL_TDUC_TEMP_AMT);
